...
> **基準：`sql/applied/` 有 148 個 `.sql`**（＋ 2 個非 SQL 的 `.ts` / `.py`，
> 全部檔案 150 個；最後歸檔的是 `2026-09-03_名次分布與平均得點.sql`）
>
...
get_my_stats_tx(p_org_id, p_member_id)                    ★ 2026-09-03 新增
  ...
  → { ok, season_from, min_games,
      season: { games, avg_rank, avg_score, ranks, national_rank, national_total, stakes[] },
      all:    { games, avg_rank, avg_score, ranks,                                stakes[] } }
  ★ 2026-09-03 補三樣（`create or replace`、簽名沒變）：
  ...
  · **`avg_score`** = 平均桌上得點。勝率只說「贏幾次」，這個說「贏多大」
    （勝率 50% 但平均 −20 ＝ 贏小輸大）。
    ⚠ 分母是**有登記分數的場次**（`avg` 自己忽略 null），不是已結算場次。

  · **`min_games` = 5（樣本數門檻）**，`stakes[].pct` **未達門檻回 null**。
    🔴 在此之前畫面會出現「**100% · 3 場**」—— 那不是成績是噪音。
      天鳳／雀魂／Chess.com Insights 全都有門檻。
    ✅ **門檻在後端不在前端** —— 放前端的話它會變成第二份規則，
      而下一個加百分比的地方一定會忘記套。門檻值一併回傳，
      前端才講得出「再 N 場看勝率」而不用自己寫死一個 5。
    ⚠ 前端判斷要用 `pct == null` **不是** `!pct` —— **0% 是一個真的成績**
      （五場全輸），`!0` 會把它誤判成未達門檻。
    ⚠ 名次分布**不套門檻**：它回的是次數，而次數在任何場數下都是事實。
  ...
  🔴 **胡牌率／放槍率／自摸率不在這裡，而且今天做不出來**：
    ...
    ⚠ **自摸率比另外兩個更深**：胡牌率只要知道「誰胡了」，
      ...
      **電子計分即使上線也給不出來**，一定要牌譜。
  📌 前端那一區共 7 格（本季）／6 格（歷史，沒有全國排名）：
    打了幾場・平均順位・平均得點・（全國排名）・胡牌率・放槍率・自摸率，
    底下再接**名次分布**與**各積分級距勝率**。
    順序刻意把**有數字的排前面** —— 三個「率」排最上面的話整塊看起來像壞了。
