...
> 是**錯誤儀表第 ⑥ 段**把它抓出來的（表 46→50、函式 185→229），不是有人記得。
> ⚠ 同時把儀表第 ⑥⑦ 段的期望值一起對上（表 50／函式 231／明確 anon 96）——
> 不改的話那兩格會從今天起永遠紅，而永遠紅的檢查會訓練人忽略整份儀表。
>
...
>
> ### 🔴 而「純娛樂 ⇒ null」這條 2026-09-20 起**有到期日了**
> 那一列的底台從 `0/0` 改成 **`30/10`**（`is_hygiene` 維持 `true`）——
> 今天不受影響（`placeholder_ranks_tx` 看的是 `is_hygiene`，先短路成 null），
> **但記分板上線那天它會算得出真的分數**。
> ⚠ 屆時 `get_my_stats_tx:76,104` 的 `final_score is not null`
>   **會靜默把純娛樂算進勝率與連勝**，而 `:62` 那句註解會變成謊話。
>   ⇒ 那一批要把總計那兩行換成 `not hygiene`（各級距那段本來就是看旗標）。
> 📄 `docs/02-POS與開桌/桌邊記分板設計.md` §4
