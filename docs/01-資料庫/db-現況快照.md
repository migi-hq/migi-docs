...
> **基準：`sql/applied/` 有 181 個 `.sql`**（＋ 2 個非 SQL 的 `.ts` / `.py`；
> 最後歸檔的是 `2026-09-06_收桌也給隨機桌上積分.sql`）
>
...
> ⏳ **目前還沒有任何地方讀它** —— 要在 App 上看到勝率／最長連勝，
>   還要改 `get_my_games_tx` 與成績頁。
>
...
比對現在 `sql/applied/` 的 `.sql` 數與上面的基準（**158**）—— **不同就是過期**。
這個檢查不需要資料庫。
