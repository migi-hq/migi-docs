...
> **基準：`sql/applied/` 有 154 個 `.sql`**（＋ 2 個非 SQL 的 `.ts` / `.py`，
> 全部檔案 156 個；最後歸檔的是 `2026-09-04_賽季排行榜與名人堂.sql`）
> ⚠ `sql/pending/` 還有 1 份：`2026-09-04_會員合併.sql`（見下面那段警告）
>
...
> **當下規模（2026-09-04 實測）：函式 168 · 資料表 47 · 檢視表 22 · RLS policy 29**
>
> ⚠ **表 47／函式 168 裡有一組是「已在線上但驗證未完成」**：
> `member_merges` ＋ `merge_members_tx`（`sql/pending/2026-09-04_會員合併.sql`）。
> 🔴 第一次跑時驗證段炸在第 ③ 格（`table_sessions` 漏了 `mode`），
> 而 **DO 區塊的 exception handler 把錯誤接住了沒往上拋 ⇒ 交易照樣提交** ——
> 所以 DDL 生效了，但那支函式的每一行邏輯只被驗到第 ② 格。
> 🎯 **那正是硬規則 7 最擔心的狀態**，重跑補完之前不要當成可用。
...
比對現在 `sql/applied/` 的 `.sql` 數與上面的基準（**154**）—— **不同就是過期**。
這個檢查不需要資料庫。
