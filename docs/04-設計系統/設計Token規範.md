...
座位卡的稱號文字目前寫死 `#B8860B`（POS `SeatPage.jsx`），是全 POS 唯一剩下的寫死色碼。

`--gold #E8B89B` 是淺金，定義為「稱號邊框、點綴」，當文字色會看不清；
硬換成 `--milktea-ink #7A5C42` 則會失去金色質感。
→ **缺一個「深金字」token**，需要定色值後再收進本表並同步三端。
