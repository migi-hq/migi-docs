...
31. 🔴 **會員 App 有三顆膠囊在說謊（顯示寫死的假資料）**（2026-08-27 盤點）。
    ...
    | `rewards.jsx` 獎勵頁 | 「我的點數 1,250」 | 寫死。**錢包頁顯示真餘額，獎勵頁顯示 1,250** |
...
36. 🔴 **換手機／換 LINE 的四種情境，其中兩種現在無解**（2026-08-28 盤點）。
    ...
    **「讓客人能改手機」不是小功能，它是唯一能預防 E 的東西。**
    ...
    ### 🔴 D 現在的三個缺口
    · **沒有任何「改手機」的 RPC** —— 唯一會寫 `phone` 的是
      `register_member_tx`（建立時）
    · `profile.jsx:193` 的手機編輯 sheet **只寫 localStorage**，
      換裝置就沒了 —— **它假裝存好了**（待辦 31 那三顆之外的第四顆）
    · ⚠ 而真正決定「換 LINE 時找不找得到他」的是資料庫裡那個舊值

    ### 🔴 E 現在完全沒有補救路徑
    | 環節 | 現況 |
    |---|---|
    | 店員搜尋 | `pos_search_members_tx` **只能用暱稱或手機** —— E 的客人兩個都可能對不上 |
    | 換綁 LINE | `rebind_line_user_tx` **存在**，但 POS 沒有 UI |
    | 誰能按 | 需要 `p_staff_id`，而店員登入還沒做（PENDING） |

    ⚠ 客人已自建第二個帳號時，`rebind_line_user_tx` 會回
      `line_user_already_bound` → 那時只剩**合併工具**（待辦 15）。
    📌 這說明為什麼店員登入（待辦 20）與帳號合併（待辦 15）是**同一個問題的兩半**。
    ...
    | 情況 | 誰能改 |
    |---|---|
    | `phone` 是 **null**（沒填過） | ✅ 客人自己補填 —— 沒有佔用風險，跟註冊時填是同一件事 |
    | `phone` **已有值** | 🔴 只有店員能改（`set_member_phone_tx`，比照 `rebind_line_user_tx` 留稽核事件） |

    ⏳ **在那之前**：個人設定那一列點下去顯示「要更改手機號碼請洽櫃檯」，
      **不要假裝存好了**。⚠ 保留可以點而不是整列變灰 ——
      一個點不下去又沒有說明的欄位，客人只會覺得壞掉。
