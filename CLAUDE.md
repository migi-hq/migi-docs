...
10. **按壓回饋：hover 有了，`:active` 還沒有**（2026-09-05 重新界定）。
    ...
    ### ⚠ 這一批**沒有實機看過結帳頁**
    build ✅、dev server 開得起來（登入頁正常，不是白畫面）✅、
    純函式測試 ✅、keycheck 24 → 0 ✅ ——
    但 **POS 要登入才進得去結帳頁，而 Claude 不能碰密碼**（硬規則 11.7）。
    🔴 所以「畫面上按 Tab 真的會跑對順序」這一格**還沒驗**。
      改動本身是純加屬性、零樣式，風險低，**但那是推論不是量測**。
      → 下次有人登入 POS 時，在結帳頁按幾次 Tab 看一眼就結案。
