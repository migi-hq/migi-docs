...
- ✅ ~~LINE Developers 帳號未申請~~ → **2026-08-28 已建立**：
  ...
  🔴 **待辦 15 的「同一個人變成兩個會員」已經真的發生了。**
  8/29 那一版寫「創辦人 = `d73fdac2`、手機 **0910768736**、
  所以他用 LINE 註冊會走 `rebound` 不會建新會員」—— **結果建了新的**。
  原因是 `register_member_tx` 用**手機**找人，而測試01 的手機是
  `0910000001` 不是 `0910768736`（那句話當時就已經不準，或之後被改掉）。

  ⚠ **一半在這邊、一半在那邊**：LINE 與驗證過的手機在新帳號，
    70 筆訂單與 3,580 餘額在舊帳號。**那正是待辦 15 描述的形狀。**
  📌 兩邊都是 `is_test`、都還沒有真實營運資料，所以現在合併成本最低。
    依待辦 15 的規則（存活者 = 歷史比較貴的那個）**應該留 `d73fdac2`**
    並把 `line_user_id` 搬過去 —— 但那需要合併工具，見待辦 15。

  🔴 **這一段是「文件會漂」的第 N 次，而且這次真的害到人**：
    2026-09-01 我照抄 8/29 這段 id 寫測試工具，**造了三場戰績給錯的帳號**，
    而畫面上什麼都沒變。同硬規則 3 與踩坑第 29 條：
    **會員 id 也要當場查，不可以抄文件。**
