...
10. **假資料是流程的一部分，不是缺陷。**（2026-08-27 使用者說明，我違反過）
    ...
    **推論：假資料的存在不構成「不能上線」的理由，但「上線」的定義是全部接完。**
    ...
    牌譜辨識（M5+，`paipu_events` 每一手摸打），那上線就在 M5 之後。
    **這是排程事實，不是要拿來說服使用者砍功能的理由。**
    🔴 **2026-09-03 更正：原本這裡寫「逐手重播與結算依賴牌譜」——「結算」是錯的。**
      牌局結算（誰胡／誰放槍／台數／連莊／桌上積分）在 `hands` 與
      `game_players` 裡，那是 **M4 電子計分**，不用影像辨識。
      ⇒ 真正被 M5 綁住的只有**逐手重播、AI 教練、吃碰槓率**三項。
...
33. **會員 App 接線進度表**（2026-08-27 全盤點）。
    ...
    **賽季膠囊的倒數**（2026-09-01，在此之前「倒數 23 天」是寫死的、從來沒變過）。
    ...
    🔴 **2026-09-03 更正：這一段原本把「胡牌率／放槍率」歸到 M5 牌譜，那是錯的。**
    ...
    | **`paipu_events`**（每一手） | 每一次摸打（`turn_no`／`action`／`tile`／`confidence`） | **M5 牌譜辨識** |
