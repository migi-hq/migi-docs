...
13. 🔴 **設計 token 一律來自 `@migi/assets`，不要在任何 repo 裡再定義一份。**
    ...
    **13.9 🌙 深色一律用 `--dk-*`（v1.13.0，2026-09-23 建立，給 `migi-table`）。**
    ...
    ```
    ⚠ `--accent`／`--accent-deep`（桃紅）、`--danger`／`--dk-danger`（紅）在平板上**都不要用** ——
      `--dk-danger` 雖然對比合格，也不用。
