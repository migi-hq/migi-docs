...
0.5 ~~當日暢打全鏈路~~ → **已完成並實機驗證通過**（2026-08-17～19，詳見已完成區）。
   ...
   **持有暢打者代付兩人收 $300**（自己免、兩人各 150）——
   ...
   🟡 **未決：會員等級折扣會折到暢打本身**（2026-08-20 實測時發現）。
   暢打是 `venue_fee` 商品，而規格是「等級折扣只折檯費」——
   所以提拉米蘇會員買暢打實收 270 不是 300。邏輯上沒錯，
   但意思是**高等級會員的暢打永遠比較便宜，而暢打本來就是為了讓人多打**，
   兩層優惠疊在同一件事上。要排除的話是 `checkout_tx` 分桶時把
   `SVC-TBL-DAY` 另計，一行的事。定價決策，不是 bug。

   🔴 **未決：跨午夜。** 現行規則是台北時區的日曆日（23:59:59 換日），
   22:30 買、打到凌晨兩點的客人，00:00 一到就開始收場地費。
   **麻將店通宵是常態，這條規則遲早現場爆。**
   世界級的日票（健身房、樂園）都是日曆日制沒錯，但那些生意不跨午夜。
   選項：購買後 12/24 小時制、或營業日制（凌晨 6 點換日）。
