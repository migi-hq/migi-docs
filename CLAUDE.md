...
3. **不要線上猜欄位名稱或約束值。** 動任何 RPC / schema 之前，先讀 `docs/` 下的權威文件，
   ...
   **含註解的全文**，字串比對分不出程式碼與說明文字。已經踩兩次：
   ...
   - 🔴 **2026-09-20 第四次，而且「函式名」這個判準沒有救到我**：
     建成就系統的 RPC，驗證段問「有沒有函式會呼叫那支不存在的發放函式」，
     掃法是函式名 —— 而唯一命中的是 `ach_unlock_tx` 裡我自己寫的
     「⚠ 這裡刻意沒有發點數 —— <那個函式名> 不存在」。
     🎯 **下面那條「禁字只用表名、函式名」在這裡失效了**：
       函式名確實不會出現在**一般**說明文字裡，
       但**「我刻意沒有用 X」這種註解一定會寫出 X**。
       ⇒ 判準要補一句：**掃「不該存在的東西」時，那個東西的名字
         通常正好會出現在「解釋為什麼不用它」的註解裡。**

   - 🔴 **2026-09-23 第五次，而且是在引用這條規則的同時踩它**：
     平板配對改成總部權限，驗證段問「這支有沒有自己比對角色欄位」，
     掃的是那個比對的寫法 —— 而唯一命中的是**我自己在同一支函式裡寫的註解**
     「權限走 can()，不自己寫 <那個寫法>」。
     🎯 **跟第四次（2026-09-20）是同一個形狀，而我在同一天內又寫了一次** ——
       「我刻意不用 X」這種註解一定會寫出 X。
     ✅ 修法：註解**用文字描述那個寫法**，不要寫出它；
       驗證段從「有沒有提到」改成「**有沒有真的拿來判斷**」
       （掃 `if`／`where`／`and` 後面接著那個欄位的形狀）。

   ### ✅ 正解不是換一個禁字，是換一種問法
   ```sql
   -- ❌ 是非題：分不出程式碼與註解
   pg_get_functiondef(oid) ~ '禁字'

   -- ✅ 逐行印出來判讀（硬規則 3.5 原本就說的做法，這裡給片段）
   select proname, (regexp_match(pg_get_functiondef(oid), '([^\n]*禁字[^\n]*)'))[1] as 命中的那一行,
          case when (regexp_match(...))[1] ~ '^\s*--' then '（註解，無害）' else '🔴 程式碼' end
   -- ✅ 或者問「有沒有**呼叫**」而不是「有沒有提到」
   pg_get_functiondef(oid) ~ '(perform|select|:=)\s+(public\.)?禁字'
   ```
   ...
   **3.6 `{/* */}` 只在 JSX 的「子元素位置」合法。**（2026-08-25 掛掉一次 build）
   ...
   ### 🔴 3.6c-2 探針字串要**先在本機產物裡找出來**，不可以憑想像寫
   （2026-09-20 踩到，而且差一點又把一個成功的部署講成失敗）
   把 `這間店的牌局` 改成 `` `${店名}的牌局` `` 之後，我挑的探針是
   獨立字面值 `"的牌局"` —— **線上 0 筆**。
   🔴 而**本機自己剛 build 出來的那份也是 0 筆** ⇒ 探針本身就是錯的：
     樣板字面值 minify 完仍然是
     ``Ie!=null&&Ie.name?`${Ie.name}的牌局`:"這間店的牌局"``，
     那幾個字**沒有變成一個獨立的字串常值**。
   ⚠ 症狀與「還沒部署」一模一樣 —— 兩者都是 `grep` 回 0。
   ✅ 正解多一步，而且很便宜：
   ```bash
   # ① 先在本機產物裡把它 minify 之後的樣子撈出來
   grep -o ".\{60\}新文案.\{10\}" dist/assets/index.*.js
   # ② 拿**那一段**去搜線上，不要拿你以為的樣子
   ```
   🎯 這是 3.6c 的前一步：3.6c 說「挑一個只在新程式碼裡出現的字串」，
     這一條說「**先確認那個字串在你自己的產物裡真的長那樣**」。
   📌 中文 UI 文案仍然是最好的探針 —— 但**要挑那個字面值的完整形狀**，
     不要挑它的一部分（`的牌局` 是 `這間店的牌局` 的一部分，也是
     `還沒有計分的牌局` 的一部分 —— 本機那份裡它出現在五個不同的地方）。
...
6. 溝通與註解一律**繁體中文**。
   ...
   **6.5 🔴 賭博紅線是用詞問題，不只是功能問題。**（2026-09-06 使用者糾正）
   ...
   「嚴禁賭博，牌桌上不能有現金交易或轉帳行為」。
   🔴 **2026-09-16 換過一次**：舊版是「嚴禁賭博，點數只用於檯費與餐飲」。
     新版把**現金與轉帳**寫進去了 —— 那才是實際會發生的事，
     而舊版只說了「點數怎麼用」。
   📌 文案在 `migi-web/src/lib/notice.js`，**全站唯一一份**
     （配桌 6 條、包桌 4 條，兩份不是一份，差別的理由寫在那支檔案裡）。
...
11. **推之前先在本機 build。**（2026-08-28 起可行）
    ...
    **沒有 staging**（硬規則 5.7 明講不做）。所以：
    ...
    ### 🔴 2026-09-20：那個預告兌現了 —— **種 localStorage 那條路已經死了**
    退回全部收掉（待辦 14 收尾），所以：
    ```
    ❌ 種 localStorage.migi_member ＋ 燒保險絲    後端只認 JWT，會被 28000 拒絕
    ✅ 走測試帳號（真 JWT）                       ← **現在是唯一的路**
    ```
    🎯 **一個「日後會壞」的警告，在寫下來的那天就該指向替代方案** ——
      而它當時就指了（那份文件 2026-09-05 就存在）。所以今天零成本。
...
13. 🔴 **設計 token 一律來自 `@migi/assets`，不要在任何 repo 裡再定義一份。**
    ...
    ### 🔴 13.4 「搬進套件」跟「有人讀套件」是兩件事
    （2026-09-21 發現，段位熊在這個狀態躺了三週）
    ```
    migi-assets/bears/          七隻段位熊 —— 正本，三端 npm install 都裝進去了
    migi-web/src/assets/        🔴 逐位元相同的副本  ← images.js import 的是這份
    migi-pos/src/assets/        🔴 又一份          ← avatar.jsx import 的是這份
    ```
    ⇒ **改正本 → bump 版本 → 三端 `npm install` → 畫面一點都不會變**，
      而且**不報錯**。套件那份從搬進去那天起就沒有任何一端在讀。
    ⚠ `exports` 的 `"./bears/*"` **一直是開著的** —— 路早就鋪好，只是沒有人走上去。

    🎯 **而 POS 的註解自己就是證據**：`avatar.jsx` 原本寫
      「同一份美術有兩個地方……真的要收，是把共用資產抽成套件，**那是另一件事**」
      —— 那個套件 2026-08-29 就建好了。
      🔴 **一句「日後再說」會一直看起來像還沒到期**，因為沒有任何東西會告訴你它到期了
      （同 `jsxcomment.py` 那個坑的反面：那次是「文件說有而沒有」，
       這次是「**東西有了而文件還說沒有**」）。

    ✅ 已接線（`migi-web` ＋ `migi-pos` 2026-09-21），副本刪除。
    🎯 **驗收用的不是「畫面看起來一樣」，是 content hash**：
      兩個 repo 各自 build 出來的七隻 hash 逐一相同（`bear-rank-bronze.e4df2bf5` …）
      ⇒ 那是「同一份位元組」的證明，肉眼比對做不到這件事。
    ⚠ 還要驗 dev（硬規則 11.5）：七隻在 dev server 全部 200，
      路徑指向 `/node_modules/@migi/assets/bears/` —— **兩種解析器都要過**。

    📌 **判準：搬進套件之後，去看「舊的那份還在不在、誰在讀」。**
      搬家沒有把舊地址退掉，信就還是寄到舊地址。
    ...
    **13.9 🌙 深色一律用 `--dk-*`（v1.13.0 建立、v1.14.0 收掉紅，2026-09-23，給 `migi-table`）。**
    ```
    --dk-bar / --dk-bg / --dk-raised / --dk-tile     四層底（由深到淺）
    --dk-text / --dk-text-2 / --dk-text-3            三階字
    --dk-line / --dk-scrim
    ```
    🔴 **不是「換 `:root`」**：既有名字是字面的（`--white`／`--ink`），
      在深色底下把 `--white` 重新定義成深灰，就是「名字說一件事、內容是另一件事」。
    ✅ `--brand`／「`--ink` 站在 `--brand` 上」在深色上都合格，直接沿用。
    ⚠ **沒有綠色**：MIGI 標準色沒有綠（使用者 2026-09-23 糾正），`--ok` 只給 POS 空桌圓點。
      「收到」「已確認」「確認鈕」一律 `--brand`。
    🔴 **也沒有桃紅、沒有紅：畫面只用粉色系 token**（使用者 2026-09-23 同日再糾正）。
    ```
    收到的積分        --brand 粉字
    付出的積分／負數   --dk-text 白字     ← 正負號已經講清楚方向，顏色只標「好消息」
                      🔴 **例外：確認彈窗裡的大數字一律 --brand 粉**，不分收付（2026-09-24 使用者選「第 2 版 標準版」）
    卡片按鈕          只有「取消」（灰框）與「確認」（粉底墨字）兩種，**一樣寬**
                      （原本確認兩倍寬，2026-09-24 改成跟第 2 版一樣）；
                      🔴 **2026-09-25 改成全有或全無**（取代原本的「各自結算」）：
                      要確認的人**全部確認才一次入帳**；**任何一人取消 ⇒ 這次操作作廢、不推進**，
                      送出的人重送一次。按完都回主頁，畫面上不寫「等某某確認」
                      ⚠ 用詞是「**這次操作作廢**」不是「這局作廢」（使用者指定）——
                        作廢的是那一次送出，那一局還在，重送的就是同一局。紀錄頁標「操作作廢」
    選中磚上的台數     --ink（它站在粉底上）
    淺底上的強調       粉色螢光筆底（--brand／--brand-tint）＋ --ink 字
    包牌格的虛線框     --brand（原本是 --gold，同日改掉 —— 金色也不在粉色系裡）
    ```
    ⚠ `--accent`／`--accent-deep`（桃紅）、`--danger`（紅）、`--gold` 在平板上**都不要用**。
      **唯一例外：MIGI 牌型格的邊框用 `--gold`**（2026-09-24 使用者指定）——
      那是會員 App 成就「傳說」金邊的同一個 token，讓招牌牌型跟招牌成就長得一樣。**別的地方不要跟著用。**
      `--dk-danger` **v1.14.0 已從套件移除**（沒有讀者了，留著會讓人以為負數該用它）。
    🔴 **每一張確認／結果卡只講這一台自己的數字**，不列另外三家各多少（使用者同日指定）——
      同「每台只顯示自己的幕前積分」：平板是一個人的，別人付多少不是他要核對的事。
    📌 現行參照：`docs/_資產/桌邊記分板_四台同步.html`（實測所有元素的計算色零命中）。
...
🔴 **不可以寫成「贏錢／輸錢」。** MIGI 是健康麻將品牌，App 的注意事項
  第一句就是「嚴禁賭博，牌桌上不能有現金交易或轉帳行為」（2026-09-16 改，見硬規則 6.5）——
...
8. ~~**後台新增商品會產生不相容貨號**~~ → ✅ **已修**（2026-09-11 查證）。
   `nextSku` 已經移除，`products.js:46` 的註解就記著
   「舊版 nextSku() 產的是兩段式流水號（SER-001），而且會掃貨號尾端最大數字 +1」。
   現在的做法是**前兩段由分類與子類自動帶（唯讀）、第三段人打**
   （`Products.jsx` 的 `skuPrefix()` ＋ `f.skuItem`），
   而前綴來自 `product_taxonomy` 主檔 ⇒ `SER-`／`SVC-` 那個矛盾從根消失。
   🔴 **這一條在文件裡過期了不知道多久** —— 又一次「文件說有、實際上沒有」。
     📌 反向版同樣成立（踩坑第 29 條）：**先查再說「有」。**
...
14. ✅ **會員端的身分：LIFF 換 JWT —— 2026-09-20 收尾完成。**

    ### ✅ 2026-09-20：前端退回全部收掉，身分只剩一個答案
    ```
    改之前   p_member_id := coalesce(public.current_member_id(), p_member_id);
    改之後   p_member_id := public.current_member_id();
             if … is null then raise '未登入…' using errcode = '28000'; end if;
    ```
    **21 支收掉，1 支刻意保留**（`log_app_event_tx`，見下）。
    📄 `sql/applied/2026-09-20_收掉身分的前端退回.sql`

    🔴 **一定要加那道拒絕，不可以只拿掉 `coalesce`** ——
      少了它，沒有 JWT 時會是 `update … where id = null` ⇒ **0 列，靜默失敗**。
      「跑了、回了、什麼都沒發生」正是硬規則 4 那個形狀。

    🔴 **`log_app_event_tx` 不是退回是條件覆寫，不可以一起收**：
      它寫的是 `case when p_member_id is null then null else coalesce(…) end`
      ⇒ POS 送 null 時保持 null。無條件改的話**每一筆 `pos_error` 會掛到
      當班店員頭上**（店員本身也是會員），而 `trg_app_events_no_mutate`
      讓它**蓋錯了永遠改不掉**。

    ⚠ **已知代價：本機「種 localStorage ＋ 燒保險絲」那條路死了**
      （硬規則 11.7 早就預告過「拿掉那個退回就死了」—— 這是那一天）。
      ✅ 改走測試帳號那條真 JWT：`docs/09-環境流程/用測試帳號登入會員App.md`

    ### 📊 收尾當天的證據（近 14 天 `member_id_src`）
    ```
    prod  · jwt     118 筆   ← 🎯 正式站 **100% 走 JWT，零 param**
    local · param   126 筆   ← 全部本機 dev（localhost 進不去 LIFF，正常）
    ```
    🎯 **那個 `host` 欄位是 2026-09-11 補的，正是為了讓這個判斷
      「自己答得出來」而不必每次做事件關聯** —— 今天它兌現了。
    ...
    ### 🔴 那 6 筆 `param` **全部來自本機** —— 而我第一次盤點時說錯了
    我原本寫「param 還沒歸零 ⇒ 退回還不能拿掉，要等新資料才知道是誰」。
    ✅ **今天就答得出來**，只是**要用 `_sid` 關聯**，不能只看那個事件自己：
    ```
    每一筆 member_id_src = param
      ↳ 同一個 _sid 的 member_session ＝ ext / no_login / **local**   ← 六筆全部
    ```
    ⇒ 那跟 `no_login/ext` 是同一件事（本機 dev，`localhost` 不在 LIFF endpoint），
      **不是真實客人靠退回拿到資料**。
    🎯 **教訓：一個埋點答不出問題時，先問「有沒有別的事件在同一個 session 裡」**，
      不要直接下「資料不足」的結論 —— 那會讓一件已經成立的事被當成待辦。
    📌 補 `host` 仍然是對的（讓它自己答得出，不必每次做關聯），只是**不必等它**。

    ### ✅ 那 5 筆 `whoami_failed` 的原因也查出來了
    ```
    14:40  v0909-1348  whoami_failed   ← 而同一秒的 member_id_src 是 **jwt**
    14:42  v0909-1348  whoami_failed
    14:48  v0909-1443  whoami_failed   ← 換版本，還是失敗
    14:54  v0909-1452  whoami_failed
    14:59  v0909-1452  whoami_failed
    16:29  v0909-1509  **ok**          ← 這一版修好了
    17:05  v0909-1702  ok
    ```
    🎯 **同一次的 `member_id_src` 是 `jwt`** ⇒ **session 有、身分也解析得出來**，
      壞的只有 `whoami` 那一支呼叫本身。
    ⇒ 那不是「拿不到 session」，是 09-09 下午一連串「改了再試」的除錯過程，
      跨了三個版本，在 **v0909-1509** 修好。之後全部 ok。
    ### ✅ 2026-09-11 02:28 實機確認：問題確實過去了
    使用者在 LINE 裡開了一次 App，留下的紀錄是：
    ```
    member_session   ok / liff / prod / **nth=1**   v0911-0218
    member_id_src    jwt / **host=prod**            v0911-0218
    ```
    · **nth=1** ＝ 第一次就成功，沒有被導去登入再回來
    · **那是第一筆帶 `host` 的 `member_id_src`** ⇒ 當天推的改動在正式站生效

    ### 🔴 而收尾判準第一版又寫成「永遠紅」，同一天改掉
    我先寫「近 7 天 liff 全部 ok」，它印 🔴 —— 因為那 5 筆 09-09 的失敗
    **還在 7 天窗口裡**，也就是它會**一路紅到 09-16**，而問題早就不存在。
    ⚠ 這是同一族的病第四次（永遠紅的檢查會讓人學會忽略紅色），
      而**這一格正是待辦 14 的收尾條件，紅錯了代價很高**。
    ✅ 判準改成**看先後**：最後一次失敗之後有沒有再失敗。
      現在印「最後一次失敗是 09-09 14:59，之後到 09-11 02:28 都沒有再失敗過」。
    ⚠ 「完全沒有樣本」要說成 ⚪ 不是 ✅ —— 沒有人開過**不是通過**，是沒有資料。
    📌 順帶：`09-10 10:57` 那筆 `jwt` 對應 `ext/no_login/prod` 並不矛盾 ——
      那是走「測試帳號用 Supabase 密碼登入」那條路（`liff.login()` 沒跑，
      所以探針記 no_login，但 Supabase session 是有的）。
    ...
    ### ✅ 待辦 21 的五項阻擋條件已全部結案（2026-09-09）
    （以下「現況」那段描述的是 2026-09 之前，保留供對照。）
    ~~`migi-web` 用 anon key，會員身分靠前端傳 `p_member_id`~~
    - ~~🔴 **存取控制**：知道任何一個會員 uuid 就能查他的錢包與消費明細~~
      → ✅ **2026-09-20 堵死**：身分一律 `current_member_id()`，
        前端送什麼 id 都被忽略。

    ### 🔴 Realtime：卡的**不是** JWT，而且 MIGI 可能根本不需要它
    （2026-09-20 重新評估，推翻這一條原本的敘述）

    這裡原本寫「Realtime 做不了，因為 RLS 是 org 級、anon 訂閱收不到」。
    **JWT 上線之後那句就過期了**，但 Realtime 仍然做不了，卡的是另外兩件事：
    ```
    match_queues / match_queue_players   🔴 兩張都是 **0 policy**
    supabase_realtime publication        🔴 一張表都沒加
    ```
    🎯 **0 policy 不是漏掉，是這個系統裡最安全的狀態** ——
      訂閱走 SELECT，沒有 policy 就是收不到。⇒ 要做 Realtime 得先**開一扇現在關著的門**，
      而那扇門不好開：配桌列表**本來就要看到別人的房**（那是功能），
      所以 policy 不能寫成「只能讀自己的」⇒ 那是隱私與功能的取捨，不是加一條 policy。

    ### 🔴 而更根本的：MIGI 的即時需求該走 **LINE 推播**不是 Realtime
    Realtime 有一個前提：**客人正開著 App 看著螢幕**。而配桌最關鍵的那一刻不是：
    ```
    19:00 報名 → 去上班／吃飯 → 21:00 湊滿了
                                  ↑ 他的 App 根本沒開，Realtime 推給誰？
    ```
    而待辦 38 自己就寫著「配桌最關鍵的一則訊息是**你的牌局湊滿了**，
    只有加了好友的人收得到」⇒ **那是 Messaging API 推播**。
    **LINE 推播不需要 App 開著，Realtime 需要** —— 對 MIGI 這是決定性的差別。
    ...
    | 通知未讀數 | ❌ 沒有 |
    | 桌況（POS） | ❌ 店員不是秒秒盯著 |
    | **記分板多裝置同步** | ✅ **有** —— 胡牌了四支手機要同時更新 |

    ⇒ **除非讓客人用自己的手機看即時分數，否則 Realtime 不用做。**
      桌邊一台平板的話它完全不需要（大家看同一個螢幕）。
    ⚠ **輪詢成本今天是 0** —— 成本 ＝ 開著 App 的人數 × 頻率，而真實客人 0 個。
      而且伺服器不是瓶頸，被吃掉的是**客人的手機電量與行動網路**。
...
19. ⚠ **`next_doc_no` 已經存在，不要再建第二套流水編號**（2026-08-23 查證完畢）。
    ...
    **而且它本來就是分店編號**（`p_store_id` 在簽名裡）—— 目前有 **7 間門市**，這點重要。
    ...
    ### 🔴 而「7 間門市」這句話會誤導人：它們是**四種**不同的東西
    （2026-09-10 實查 `stores.store_type`，在此之前只寫「7 間」）
    | `store_type` | 幾間 | 哪些 |
    |---|---|---|
    ...
    | **自家場** | 1 | 永康雀友場 `S05` |
    ```
    七間全部 is_test = true      ← 一間都還沒真的營運
    有桌位的只有兩間              高雄自由 14 桌 · 前鎮 4 桌
    其餘五間                      0 桌 0 場 0 訂單
    ```
    🔴 **我 2026-09-10 寫過「七間店裡只有一間在營運」—— 兩處都錯**，
      而且是沒查就寫（硬規則 3）。正確的說法是「**一間都還沒營運**，
      而有資料的是兩間」。
    🎯 **這個分類會咬人的地方是庫存與帳務**：加盟／系統授權／自家場那四間
      **不是你的庫存也不是你的營收**。

    ### ✅ 調撥要做，界線是**法人**不是店數（2026-09-10 使用者指正）
    🔴 我一度把「多倉調撥」寫進「永遠不要做」，**那也是錯的** ——
      同法人底下不同分店之間搬貨是真實需求（這家快過期、那家缺貨）。
    **判準一句話：`from_store.entity_id = to_store.entity_id`。**
    而這條線**資料模型早就畫好了**（實查 `legal_entities`）：
    ```
    MIGI 總部（kind=hq）        → 3 間直營 S01 / S02 / S04   ← 可互相調撥
    苓雅加盟企業社（franchise）  → 1 間 S03
    西屯加盟企業社（franchise）  → 1 間 S06
    永康雀友場（franchise）      → 1 間 S05
    大雅授權商（licensed）       → 1 間 S07
    ```
    ⇒ 今天「可以互相調撥的店」正好就是那三間直營，其餘四間各自獨立法人、
      各一間店，本來就沒有對象可調。

    🔴 **跨法人那條線要在後端擋，而且錯誤訊息要講出為什麼**：
    ```
    不同 entity_id → { ok:false, reason:'cross_entity',
                       message:'不同法人之間不是調撥，那是一筆銷售，請開訂單' }
    ```
    ⚠ **「調撥」這個功能名稱本身會誤導人** —— 第一個用它的人很可能拿它把貨
      調給加盟店，而**系統不會報錯，只會讓兩邊的帳都錯**
      （總部少了庫存卻沒有收入，加盟商多了貨卻沒有成本）。
    ⚠ 調撥在異動流水裡是**兩筆**（`transfer_out` ＋ `transfer_in`，共用同一張
      單據 id），不是一筆帶正負號 —— 後者會讓兩邊的結存都算不出來。
    📄 詳見 `docs/07-營運商業/進銷存系統設計.md` 第五之一節與《加盟帳務設計》。
...
20. 🔴 **店員登入：身分是「有 LINE 的會員 + staff 表一列」**（2026-08-23 查證定案）。
    ...
    ### ✅ 第二次：`admin_remove_avatar_tx`（2026-09-10 挖到並修掉，同一族）
    `sql/applied/2026-09-10_補上總部下架頭像的權限檢查.sql`，**7/7 全過**。
    ✅ **提交之後另外查過線上**（硬規則 1.8）：版本數 1、權限判斷在、
      `authenticated` 還在（後台沒被打壞）、零權限判斷的 `admin_` 函式 0 支。
    📌 ⑦ 那個負對照從 **7 → 8** —— 差額正是這一支，**算式對得上**
      （硬規則 3.56：期望值要能用算式解釋，不是「看起來合理」）。
    ```
...
33. **會員 App 接線進度表**（2026-08-27 全盤點）。
    ...
    **莊家與連莊由系統自己追蹤**。
    ...
    | 獎勵 · 成就牆 | ✅ **後端 2026-09-20 全部完成**，見下 |

    ### ✅ 成就系統：後端做完了，只剩前端（2026-09-20）
    ```
    引擎      ach_unlock_tx／ach_progress_tx／ach_streak_tx／fire_event_tx／ach_meta_tx
    匯入      階段 1 共 60 枚（A 新手引導 29 ＋ C 牌型收藏 31），18 枚 active
    發射端    17 枚可解鎖的**全部接上**，而 C 區 31 枚一個都還沒有（要等電子計分）
    ```
    🎯 **只有 `settle_session_tx` 是改函式，其餘 13 個事件全走觸發器** ——
      那不是偏好：寫 `team_members` 的函式有**四支**，改函式會讓另外三條
      入團路徑永遠不發事件而且不報錯。
    🔴 **驗收那一格是結構性掃描不是清單**：問每一枚 active 成就
      「全庫有沒有函式在 `fire_event_tx(…'你的事件名'` 裡寫出你」，
      配一個負對照（C 區不可以有）。⇒ 下一枚忘了接發射端時它會自己變紅。
    📌 C 區那 31 個 `pattern_*` 事件名**就是 `hands.patterns` 的字彙契約** ——
      刻意在記分板之前寫下來，因為那份資料事後補不回來。
    ⏳ **前端還沒接**：`ACH` 陣列裡的 `on: 1` 仍是逐一寫死的，
      每個會員看到的已解鎖徽章一模一樣。
    📄 `docs/03-會員App與社交/成就事件對照表_階段1.md`（發射端契約）
    ...
    | 功能 | 現況 |
    |---|---|
    | 牌咖團 | ✅ **後端 2026-09-11 ＋ 前端 2026-09-12 完成**。<br>⏳ 只剩**團徽上傳照片**沒有 UI（`uploadTeamCrest` 與 Edge Function 都在，沒有呼叫點）。<br>🔴 **牌咖團等同公會**（使用者拍板），`團員 · N/4` 已拿掉。<br>🔴 「一起打了 N 場」＝**整桌都是團員**才算（使用者推翻我提的「兩位以上」）。 |
    | 預約包桌 | ✅ **整條線 2026-09-12 完成**：後端 09-11 ＋ 會員端 ＋ **POS 包桌預約頁**。<br>POS 側邊欄「現場」第三項，一次看一天，可帶到桌／標記沒來或取消。<br>⚠ 「帶到桌」**只標記預約並記下桌號，沒有開桌** —— 檯費仍走開桌設定 → 結帳。<br>⏳ 訂了之後團員不會收到通知（刻意分開，先想清楚會不會變噪音）。<br>🔴 **預約佔的是「容量」不是某一張桌**，當天店員才指定。<br>🔴 `team_id` **可空** —— 沒有團的客人也要訂得到，否則櫃檯會另外發明一套。<br>🔴 前端**一條後端規則都沒有抄過來**（30 分鐘／90 天／時長／桌數／<br>重複／容量），只做「只給得出合法選項」＋「把後端那句話原樣顯示」。<br>📄 完整脈絡：`docs/03-會員App與社交/牌咖團與包桌.md` |
...
39. **原生對話框全站盤點**（2026-08-29 做完盤點，migi-web 的 confirm 已清）。
    ...
    ### ✅ 2026-09-11 重新盤點：admin 已清，migi-web 剩 6 處而且刻意留著

    | repo | 現況 |
    |---|---|
    | migi-pos | **0** ✅ 它有自己的 `Modal` |
    | migi-admin | **0** ✅ `ConfirmModal` 已取代（`lib/ui.jsx`）—— 這一條原本標著 🔴 |
    | migi-web | **6**（1 `prompt` ＋ 5 `alert`），**全部刻意留著**，見下 |

    🔴 **原本記的「migi-web 9 處＝1 prompt ＋ 8 alert」數錯了** ——
      實際是 **1 ＋ 9**（`wallet.jsx` 有兩個）。
    ✅ **已換掉 4 個**（`migi-web` e522460）：派車／叫車／儲值 ×2 → `showToast`。
      ⚠ 用的是**各頁已經在用的那一支**（`wallet.jsx:519` 早就在用），
        不是引進第二種做法。

    ### 🔴 剩下 6 處刻意不動，而理由比原本記的更明確
    · **牌咖團那 5 處**（`buddies.jsx` ×4、`components.jsx` ×1）——
      整組要接後端（待辦 33），接的時候那些 alert 本來就會消失。
      **現在改等於改兩次。**
    · **改團名那個 `prompt`**（`buddies.jsx:106`）——
      原本這一條說「該現在改，它有現成元件」，而 `EditTextSheet` 確實
      export 了（`rewards.jsx:1743`，`profile.jsx` 已經在用）。
      🎯 **但它操作的 `teams` 是 `useState([寫死一團])`** ——
        改團名只改本機 state，重整就沒了。
        ⇒ 換元件只是讓一個**假功能**長得好看一點，而它會跟著牌咖團重寫。
...
40. ✅ **稱號改由後端從事實算出 —— 2026-09-22 完成**
    （`sql/applied/2026-09-22_稱號改由後端認定.sql` 10/10 ＋ 行為 6/6，提交後查過線上）。
    ```
    _member_titles(member)   唯一一份定義：新手上路 ＋ 解鎖成就的 grants_title
                             ＋ 賽季冠軍「<年> <季>季雀神熊」＋ member_app_state.titles（留給後端發的來源）
    get_my_titles_tx()       App 稱號抽屜讀這支（owned ＋ 看得到的 locked）
    set_my_title_tx          改問 _member_titles
    save_app_state_tx        簽名不變，**稱號參數一律忽略**
    ```
    · 21 個收藏套組稱號已拍板（`docs/03-會員App與社交/成就與稱號設計.md` §6.5），
      **線上只填了 2 枚**（onboarding_29 新手村制霸、migi_02 MIGI）——
      🔴 **其餘 19 枚成就匯入時要一起填 `grants_title`**，不填的話稱號永遠不會出現而且不報錯。
    · 稱號永久：成就下架不收回（`_member_titles` 刻意不過濾 `deleted_at`）。
    · 隱藏成就的稱號不列在「未解鎖」—— 列了等於劇透。
    ⏳ contract：舊版前端不送 `p_titles` 之後，`save_app_state_tx` 可以 DROP 掉那個參數
      （要補 GRANT，硬規則 2）。

    （以下為原始記錄。）
    🔴 **稱號解鎖目前是「前端說了算」**（2026-08-30 查證，**不是**要現在做解鎖機制）。
...
44. **品項規格與進銷存**（2026-09-10 拍板，起點是「世界級 POS 怎麼顯示品項單位」）。
    📌 預覽：`docs/_資產/品項單位_定案.html`（卡片與購物車列從 POS 逐項抄）

    ### ✅ 第 1 步做完了（2026-09-10）
    ```
    sql/applied/2026-09-10_品項規格spec.sql   8/8 全過，**提交後另外查過線上**
    migi-pos     7cca580   商品卡＋購物車印 spec；分頁改讀子分類
    migi-admin   ffc1289   商品編輯加規格欄位
    ```
    · `products.spec` ＋ `order_items.spec` 快照，`checkout_tx` **查主檔蓋章**
    · `admin_upsert_product_tx` 改簽名 ⇒ DROP 重建 ⇒ **GRANT 補回去了**
      （驗證第 ⑦ 格就是盯這個：只驗「函式在」的話，一支沒人叫得動的
       函式也會讓其他格變綠，而後台會當場壞掉）
    · `p_spec` 放參數列最後並給預設值 ⇒ **expand-safe**，可以在前端部署前先跑

    ### 🔴 POS 的商品分頁改讀 `subcategory` —— 而 `display_group` **不做**
    （2026-09-10，使用者要求「POS 要分飲料 甜點 炸物 零嘴 主食」）
    ```
    改之前   catLabel(p.category)   只有 餐飲／周邊／服務 三值
             ⇒ 首店那 32 個餐飲品項全部擠在一個分頁裡
    改之後   subLabel(p.subcategory)  飲料 · 甜點 · 炸物 · 零嘴 · 主食
    ```
    🔴 **《首店餐飲籌備》第 10-1 節提議的 `display_group` 欄位不要加** ——
      `subcategory` 已經在回答「這是哪一類」了，再加一個就是
      **一個事實兩個名字**（這個專案記過七次的病，而且第七次是當天親手造的）。
    ⚠ 那份文件說它們「真的會分岔」，舉的例子是**堅果的 `category='fnb'`
      但客人心裡是零嘴** —— 那是 `category` 與分頁分岔，不是 `subcategory`。
      實際上 `subcategory` 就是 `SNK 零嘴`，一模一樣。
    🎯 **真正會分岔的是出餐站**（蛋塔歸甜點卻走廚房）——
      那是**第三個維度**，跟分頁無關，日後要做仍然要獨立一欄。
    📌 順帶移除 `catLabel`（唯一呼叫點就是那個分頁）。
      待辦 0 的成果沒有丟：`subLabel` 讀的是同一張 `product_taxonomy`。
    ⚠ 沒填子分類的商品用空字串接住 ⇒ 落到「其他」分頁。
      直接跳過的話那些商品會**從畫面上消失而且不報錯**。

    ### ✅ 首店 32 項建檔完成（2026-09-10，11/11 全過並複查線上）
    `sql/applied/2026-09-10_首店餐飲32項建檔.sql`（來源：《首店餐飲籌備》第 1 節）
    ```
    DES 7　DRK 8　FRY 6　MEAL 10　SNK 1  ＝ 32
    分頁順序  飲料 · 甜點 · 炸物 · 零嘴 · 主食      ← 實機確認過
    ```
    ⏳ **還有一份收尾在 `pending/`**：`2026-09-10_清掉兩個手打的庫存數字.sql`
      —— 32 項裡 30 項是 0、水餃與厚片是 50，而那 50 沒有任何東西會扣它。

    ### 🔴 商品價格印 `$` 不印「點」（2026-09-11，使用者指出）
    結帳頁右邊的小計、尚需支付、找零全是 `$`，而商品卡寫「70 點」。
    🎯 **「點」在這個系統裡專指錢包餘額**（座位卡的「73 點」、身分列、
      點數折抵那一列）—— 一個字兩個意思，而其中一種是錢的單位。
    ⚠ 後果不只是不整齊：**店員報價會直接唸成「七十點」**，
      而客人聽到的是「我要用點數付」。
    📌 同「三種分不可以互相借用」那一節：**錢包＝點數**，其餘都是新台幣。

    ### ✅ 消費明細與收據帶規格（2026-09-11，5/5 並複查線上）
    `sql/applied/2026-09-11_消費明細帶規格.sql` ＋ `migi-pos` 8e824da。

    🔴 **提議時我說錯了一句：「客人會看到水餃 10 顆／份」——不成立。**
    查了三個呼叫點的前端才確定：
    ```
    POS 收據 OrderDetail      ✅ 逐列印品項  ← 今天唯一看得到規格的地方
    POS 會員查詢「最近消費」    ⚪ 摘要一行（items[0].name ＋「等 N 項」）
    會員 App 消費明細          ⚪ 摘要一行，**寫法與 POS 那一支逐字相同**
    ```
    ⚠ 會員 App 要真的列出品項需要一個展開明細的 UI，**不在那一批**。

    ### 🔴 順帶挖到兩份逐字相同的手抄定義
    ```
    _member_orders_core            ← pos_member_orders_tx ＋ get_my_orders_tx 共用
    get_session_member_orders_tx   ← 收據，**沒有共用 core**
    ```
    兩邊各自組 `'name', i.name, 'revenue_type', …, 'qty', i.qty, …`。
    🎯 **只改一支就會讓它們分岔**，而下一個人不會知道哪份才對 ——
      所以兩支一起改，用同一個樣式跑迴圈，任一支換不到就整份回滾。
    ⏳ 真正的修法是**讓收據那支也呼叫 core**，但兩支的其他部分不同
      （一支吃 session、一支吃分頁游標），合併要逐行對照。
      **先讓它們不漂，再談合併。**

    ### 🔴 收據上會有兩種列，而那不是 bug
    **89 筆**舊快照的 `name` 裡還帶著「（10 顆／份）」，而 `spec` 是 null。
    ```
    舊　水餃（10 顆／份）　×1
    新　水餃　10 顆／份　×1
    ```
    ⚠ **不可以回填** —— 快照存的是「當時的品名」，改了就是偽造歷史。
      那份 SQL 的第 ⑤ 格就是在盯「有沒有人去回填」。
    🔴 **價格 28/32 是暫定的** —— 那份文件只給了 4 個單一數字
      （焦糖布丁 60／提拉米蘇 90／巴斯克 90／拼盤 150），其餘是**區間**
      （60–90 這種）或**完全空白**（飲料 8 項一項都沒有）。
      每一列都標了來源（`文件`／`區間下限`／`區間取中`／`暫定`）。
      ⚠ 「定價表定案」本來就還在那份文件的待辦裡。**開幕前逐項核一次。**
      🟢 改價成本是零：後台改完立刻生效（`checkout_tx` 即時查主檔）。
    🔴 **`stock_qty` 與 `unit_cost` 一律 0** —— 進銷存不存在。
      既有的「水餃 50、厚片 50」就是手打的裝飾，賣掉 202 筆之後一次都沒動過。
      **不要填一個看起來合理的數字**：一個永遠不準的數字會訓練人忽略它。
    📌 順帶：`FNB-MEAL-TOAST` → `FNB-DES-TOAST`（厚片吐司在文件裡屬甜點）。
      改貨號安全 —— `order_items` 存快照，而它 `is_system = false`。

    ### 🔴 查出來的兩件事改變了題目
    ```
    ① 「份」已經寫死在購物車裡：S.lineUnit 印 `${it.price} / 份`
       套用在每一個商品上，而且只在數量 > 1 時出現
    ② 進銷存**完全不存在**：會扣 stock_qty 的函式 0 支、
       進貨／異動／供應商的表一張都沒有
       水餃 50 · 厚片 50 是後台手打的，賣掉 202 筆品項一次都沒動過
    ```

    ### ✅ 定案：`products` 加**一欄** `spec`（`10 顆／份`，可空）
    · 商品卡：品名下面一行小字（`--gray-2`、11.5px）
    · 購物車：`S.lineUnit` 改印 `spec`，**拿掉 `qty > 1` 的條件**（永遠顯示）
    · 沒有規格的商品（厚片）**整行不畫**，不留空位
    · 🔴 **數量維持裸數字，不要寫「2 份」**

    🎯 **數量那一格是使用者擋下來的**（「總覺得很怪」），而他是對的：
      Square／Toast／Lightspeed 的購物車數量都是裸數字，
      **單位詞只用在秤重計價**（0.5 lb）——那個數字沒有單位才沒有意義。
    ⚠ 而我原本主張「孤零零的 2 有歧義」**被同一批改動自己推翻了** ——
      歧義是被規格那一行消掉的，而規格現在永遠顯示。
    🔴 連帶：原本要加的 `unit` 欄**砍掉**。數量不印單位詞之後它沒有讀者，
      建了就是「建了沒人讀」。要賣杯裝飲料而且需要「1 杯」時再加。

    ### 🔴 `order_items` 也要加 `spec`，而且是今天加
    它現在快照 `name` / `unit_price` / `revenue_type`，**沒有 spec**。
    ⇒ 雞塊從 6 個改成 10 個之後，歷史訂單只留著「2 份」，
      **那 2 份是幾個永遠算不回來** —— 而那正是進銷存第一個要問的數字。
    ⚠ 這一半是**不可回溯**的（硬規則 5.6），今天加零成本，事後補補不回來。
    ⚠ 蓋章時**回查主檔不採信前端**（同待辦 2）。

    ### ⚠ 「後台能設 10 個 1 份 / 6 個 1 份」有兩種意思
    | | 是什麼 | 怎麼做 |
    |---|---|---|
    | **A 改規格** | 雞塊就一種，店家決定一份幾個 | 編輯 `spec`，一個欄位 |
    | **B 兩種同時賣** | 6 個裝 $50 與 10 個裝 $80 並存 | **兩個 SKU**，跟檯費那七支一樣，系統已支援 |

    🔴 **B 一做 `stock_qty` 就會說謊**：兩個 SKU 各扣各的，而雞塊是同一批貨。
      進 100 個、賣 5 份 6 個裝，實際少 30 而系統只減 5。

    ### 🔴 配方（BOM）是最後一步不是第一步
    📄 **完整的七層與最小版：`docs/07-營運商業/進銷存系統設計.md`**
    ⚠ **不要在對話裡重新推導那一份** —— 它已經把「世界級有哪些功能」
      與「MIGI 實際需要哪幾樣」分開寫完了。
    ```
    ① 現在        products.spec ＋ order_items.spec 快照
    ② 要賣兩種份量  開第二個 SKU
    ③ 真的要進銷存  異動流水 → 進貨 → 盤點 → 損耗 → 換算率 →（才輪到）配方
    ```
    ⚠ 而**沒有進貨與盤點的 BOM 比沒有更糟**：它會算出一個持續偏離現實的
      帳面庫存（耗損、試吃、報廢都不會自己進系統），
      而**一個永遠不準的數字會訓練人忽略它** ——
      同硬規則 3.5 那個「永遠紅的檢查」的形狀。
    🎯 ③ 的第一個問題不是技術是**「誰收貨、誰盤點、多久盤一次」**（硬規則 5.5）。

    ### 🔴 而「要不要 BOM」看品項型態，不是看規模
    《首店餐飲籌備》寫著**首店不設廚師、主食全是整份冷凍料理包**，
    而**飲料吧台現調** ⇒ 答案直接被決定：
    ```
    整份料理包（主食／炸物／冷凍甜點）  1 包 ＝ 1 份 ⇒ 只要進出庫
    需分切（巴斯克／千層）              換算率（1 個 ＝ 8 片）
    大包裝分裝（零食小陶碟）            換算率
    🔴 現調飲料                        ← 這才是 BOM
    ```
    ⚠ 我 2026-09-10 先講「世界級的解法是配方 BOM」——**那是把答案講得比
      問題大**。一包 100 顆水餃拆成 10 份是**換算率不是配方**。
    🎯 **配方要等飲料吧台真的營運才出現**，而且只涵蓋飲料那一類。

    📌 **進銷存放 `migi-admin`** —— 待辦 18 早就拍板，不要重新討論。
      ⚠ 唯一的細節：**盤點是在店裡拿平板走動做的**，
        那一頁仍屬後台，但版型要能在平板上用（同待辦 10 的交集判準）。
    🎯 **它有一個今天就存在的商業驅動力**：《首店餐飲籌備》自己寫著
      「冷凍容量是最容易低估的硬體……每項每日預估份數 × 進貨週期
      = 所需包裝體積」⇒ **「每項每天賣幾份」正是進銷存的第一個產出**，
      而那個數字決定冷凍櫃要買幾台。不是抽象的管理需求，是會花錢的決定。

    ⚠ **檯費那七支不要動**（`配桌檯費 · 3 將` 這一類）——
      它們是**變體不是規格**，不同 SKU 不同價，寫在名字裡是對的。
    📌 `水餃（10 顆／份）` 的品名要同時改成 `水餃`，
      規格搬進 `spec` 之後留在名字裡就是兩份。

45. ⏳ **成就徽章：29 枚還沒有圖**（2026-09-23 從線上成就 × `images.js` 的 `ACH_ART` 比對出來）。
    ```
    牌型 27 枚   tile_01–06、09、13–32（字牌刻子七合二之後剩的 26 枚 ＋ 牌型收藏家）
    旗艦  2 枚   migi_01 MIGI、migi_02 十次 MIGI
    ```
    現在全部只有 emoji（`ACH_ART` 裡寫的是 `{ emoji }` 不是 `{ img }`）。
    ⚠ **數字要當場比對，不要抄這一行** —— 匯入階段 2 之後會變。比對法：
...
46. 🔴 **系統沒有「刪除帳號」，只有「隱藏」**（2026-09-25 使用者拍板，同日改了三次才定案）。
    ```
    誰能隱藏   會員自己（App 個人設定最後一列）＋ 總部（後台「會員」頁）
    誰能恢復   **只有總部**（客人到櫃檯說，店員轉給總部）
    隱藏之後   他打不開 App（current_member_id() 認不出他、開機回 hidden 畫隱藏頁）
               排名消失、牌咖名單消失、別人看到的他是問號「隱藏的會員」
               點數保留，到店裡照樣能用（POS 用會員 id 結帳，不經過他的身分）
    ```
    🎯 **做法是「搬進保險箱」不是「每一支加判斷」**：對外顯示會員的函式有 20 多支，
      隱藏時把名字／頭像／自介／稱號等搬進 `member_hidden`，原位換成問號
      （`avatar_source = 'hidden'`）⇒ 那 20 多支**一行不改就全部變問號**，恢復時原樣搬回。
      只有「要整個消失」的幾支另外濾：排名核心、名人堂、牌咖名單、最近同桌、送牌咖邀請。
    ⚠ **新寫一支「列出會員給別人看」的函式時**：問號是自動的，但如果那個畫面該讓他
      **整個消失**（排行、推薦、可加好友），要自己加 `hidden_at is null`。
    ⚠ 不要再做刪除：帳務依法保存，而會員那一列被 33 個外鍵指著。
      📄 `sql/pending/2026-09-25_會員隱藏帳號.sql`（檔頭有完整規則）
