...
## 會員 App 的狀態標籤：進行中 → 等待結算 → 第 N 名

（2026-09-06 定案）歷史配桌紀錄那個清單。
```
配桌中     還在等人
即將開始   已帶到桌、還沒開打（activated_at 是 null）
進行中     已經開打（activated_at 有值、場次還開著）
等待結算   已收桌，還沒有成績          ← M4 電子計分之前**所有**收過的桌都在這
第 N 名    結算完                     ← M4 之後才會出現
未成桌     沒湊滿四人
```
🔴 **修之前「進行中」與「等待結算」是反的**：還在打的那一列寫「等待結算」，
