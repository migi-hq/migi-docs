...
📌 也因此**各級距「勝率」停用到 M4** —— 桌上積分在資料庫裡根本沒有欄位。
...
9. **資料層例外收斂：`migi-web` 做了一半，`migi-admin` 還沒開始**（2026-08-21 更正）。
    ...
    | | web | POS | **admin** |
    |---|---|---|---|
    | 資料層例外收斂 | ✅ `rpc.js` | ✅ `api.js` | 🔴 **沒有** |
    | Toast | ✅ | ✅ `toast.jsx` | ⚪ 沒有（**刻意**） |
    | 錯誤上報／埋點 | ✅ | ✅ | 🔴 **沒有** |
    | `@migi/assets` | v1.5.0 | v1.5.0 | **v1.3.0** |
...
33. **會員 App 接線進度表**（2026-08-27 全盤點）。
    ...
    **莊家與連莊由系統自己追蹤**。
    ...
    | 成績 · 平均勝率・最長連勝・單場最多積分・積分走勢 | **桌上積分** → M4（`score_delta`／`final_score`）<br>🔴 今天沒有欄位：`score_points` 裝的是段位分（見名詞表） |
    | 牌咖 · 結算 | 牌局分數 → M4 |
