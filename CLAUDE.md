...
3.5 ⏳ **會員折扣只折檯費：已上線，但實機驗證未完成**（2026-08-17）。
   SQL 已執行（版本數 1 / 新算式存在 / 舊算式已移除 皆通過），
   但當時能看到的訂單全是**純檯費單**，新舊算式在那種單上結果相同，證明不了。
   → **需要一筆改後的混合單**（測試01 提拉米蘇，檯費 150 + 水餃 80，折扣應為 −$15）。
   驗證查詢在 `sql/applied/2026-08-17_會員折扣只折檯費.sql` 結尾，已改成只列混合單。
