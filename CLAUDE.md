...
36. 🔴 **換手機／換 LINE 的四種情境，其中兩種現在無解**（2026-08-28 盤點）。
    ...
    **「讓客人能改手機」不是小功能，它是唯一能預防 E 的東西。**
    ...
    ### ✅ D 與 E 的後端都做完了（2026-08-30 傍晚）
    · ✅ ~~沒有任何「改手機」的 RPC~~ → **`set_member_phone_tx` 建好了**
    · ✅ ~~E 完全沒有補救路徑~~ → **`claim_member_by_phone_tx`（自助認領）**
    · ✅ ~~手機編輯 sheet 假裝存好了~~ → 已止血（2026-08-28，`migi-web` 01166d9）
    ⏳ **只剩前端**：個人設定那一列還是「請洽櫃檯」，
      要改成顯示 `phone_masked` ＋ 已綁定／未綁定 Pill ＋ 走驗證流程。
      （後端 `get_my_profile_tx` 的 `phone_masked` / `phone_verified` 早就在了。）
