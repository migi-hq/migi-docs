...
33. **會員 App 接線進度表**（2026-08-27 全盤點）。
    ...
    **莊家與連莊由系統自己追蹤**。
    ...
    | ~~成績 · 積分走勢~~ | ✅ **2026-09-07 接通**，同日與段位走勢收成一塊 `lib/trend.jsx`：指標 chip（段位分／積分）＋ 期間下拉（全部／本月／本週）＋ 右上合計。<br>🔴 下拉**沿用縣市／區域那套原生 `<select>`**（App 唯一既有的下拉），曾自己畫一顆、當天撤掉 —— 不要做第二種。<br>純娛樂不畫；M4 換成真積分時一行都不用改 |
    | 牌咖 · 結算 | 牌局分數 → M4 |
