...
33. **會員 App 接線進度表**（2026-08-27 全盤點）。
    ...
    **莊家與連莊由系統自己追蹤**。
    ...
    | 預約包桌 | ✅ **後端 2026-09-11 ＋ 會員端前端 2026-09-12 完成**。<br>🔴 **還缺最後一哩：POS 當天的預約清單**（`pos_list_bookings_tx` /<br>`pos_seat_booking_tx` 後端都在，店員端零畫面）——<br>**沒有它，客人訂了到現場櫃檯還是看不到**。<br>⏳ 訂了之後團員不會收到通知（刻意分開，先想清楚會不會變噪音）。<br>🔴 **預約佔的是「容量」不是某一張桌**，當天店員才指定。<br>🔴 `team_id` **可空** —— 沒有團的客人也要訂得到，否則櫃檯會另外發明一套。<br>🔴 前端**一條後端規則都沒有抄過來**（30 分鐘／90 天／時長／桌數／<br>重複／容量），只做「只給得出合法選項」＋「把後端那句話原樣顯示」。<br>📄 完整脈絡：`docs/03-會員App與社交/牌咖團與包桌.md` |
