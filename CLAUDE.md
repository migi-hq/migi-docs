...
33. **會員 App 接線進度表**（2026-08-27 全盤點）。
    ...
    **莊家與連莊由系統自己追蹤**。
    ...
    | 功能 | 現況 |
    |---|---|
    | 牌咖團 | ✅ **後端 2026-09-11 完成**（三張表 ＋ 22 支函式，詳見 db-現況快照）。<br>⏳ 前端仍是 `useState([寫死一團])`，`social.js` 還沒有 team 函式。<br>🔴 **牌咖團等同公會**（使用者拍板），`buddies.jsx:136` 那個 `團員 · N/4` 是錯的要拿掉。<br>🔴 「一起打了 N 場」＝**整桌都是團員**才算（使用者推翻我提的「兩位以上」）。 |
    | 預約包桌 | ✅ **後端 2026-09-11 完成**（`bookings` ＋ 9 支函式，詳見 db-現況快照）。<br>⏳ 前端還沒接 —— 那個抽屜仍是 disabled 的「即將推出」。<br>🔴 **預約佔的是「容量」不是某一張桌**，當天店員才指定。<br>🔴 `team_id` **可空** —— 沒有團的客人也要訂得到，否則櫃檯會另外發明一套。<br>📄 完整脈絡：`docs/03-會員App與社交/牌咖團與包桌.md` |
