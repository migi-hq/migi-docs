...
import { StorePicker, MapPin, deco } from '../lib/components.jsx'
...
import { popPendingTableInvites, addPendingTableInvite, pendingTableInvites, sendTableInvite, fetchMatchQueues, fetchMatchQueuesByCity, createMatchQueue, joinMatchQueue, fetchStakes, fetchStores, fetchMyQueue, leaveMatchQueue, fetchNotifs, onSocial, respondTableReq, fetchBlocks, unblockMember, fetchBuddies } from '../lib/social.js'
import { track, snapQueue } from '../lib/analytics.js'
import { rankBearSrc, fmtType, fmtRank, cityFromLatLng, distanceKm, fmtDistance, isStoreOpen, storeTypeLabel, storeArea, fmtTime } from '../lib/helpers.jsx'
...
const freqLabel = (q) => {
  if (q?.recurring_freq === 'daily') return '每天開桌'
...
const TagPill = ({ children }) => <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 11, fontWeight: 600, color: 'var(--ink)', background: 'rgba(255,255,255,.7)', padding: '2px 9px', borderRadius: 'var(--r-pill)' }}>{children}</span>
const FreqPill = ({ q }) => <span style={{ display: 'inline-block', background: 'var(--accent)', color: 'var(--white)', borderRadius: 'var(--r-pill)', padding: '3px 11px', fontSize: 11, fontWeight: 700 }}>{freqLabel(q)}</span>

/* —— 牌局卡（方案一：店頭分隔式）——
   showStore=true 時上方多一列店頭（📍店名＋距離＋查看門市 ›，點擊開啟門市抽屜）；
   inStore（在門市抽屜內時）省略店頭列，避免跟抽屜標題重複。*/
function GameCard({ q, kind, store, stakeMap, myCoord, onJoin, onOpenStore, showStore = true }) {
  const isFix = kind === 'fix'
  const km = (showStore && myCoord && store) ? distanceKm(myCoord.lat, myCoord.lng, store.lat, store.lng) : null
  const need = (q.seats || 4) - (q.players || 0)
  const tags = Array.isArray(q.tags) ? q.tags : []
  return (
    <div style={{ border: isFix ? '1.5px solid var(--milktea-bd)' : '1.5px solid var(--brand)', background: isFix ? 'var(--milktea-light)' : 'var(--brand-light)', borderRadius: 'var(--r-card)', overflow: 'hidden', marginBottom: 8 }}>
      {showStore && store && (
        <div onClick={() => onOpenStore && onOpenStore(store)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 13px', borderBottom: '1px solid rgba(46,43,44,.18)', cursor: 'pointer' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>
            <MapPin />{store.name}{km != null && <span style={{ fontWeight: 500, color: 'var(--gray-1)' }}> {fmtDistance(km)}</span>}
          </span>
          <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', whiteSpace: 'nowrap' }}>查看門市 ›</span>
        </div>
      )}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {!isFix && q.source === 'pos' && (
            <div style={{ marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              <RecmdBadge />{tags.filter((t) => QUEUE_TAGS[t]).map((t) => <span key={t} style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 600 }}>{QUEUE_TAGS[t]}</span>)}
            </div>
          )}
          {isFix && <div style={{ marginBottom: 5 }}><FreqPill q={q} /></div>}
          <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)' }}>{fmtPlayAt(q.play_at)}{stakeMap[q.stake_level_id] ? <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)', marginLeft: 8 }}>積分：{stakeMap[q.stake_level_id]}</span> : null}</div>
          <div style={{ display: 'flex', gap: 5, marginTop: 6, flexWrap: 'wrap' }}>
            {fmtType(q.game_type, q.flower).split(' · ').map((x) => <TagPill key={x}>{x}</TagPill>)}
            {q.rounds && <TagPill>{q.rounds}</TagPill>}
          </div>
          <div style={{ fontSize: 12, color: 'var(--gray-1)', marginTop: 6 }}>目前 {q.players || 0}/{q.seats || 4}，還差 {need} 位成桌</div>
        </div>
        <button className="ibtn" onClick={() => onJoin(q)} style={{ background: 'var(--ink)', color: 'var(--white)', borderRadius: 'var(--r-pill)', padding: '7px 16px', fontSize: 'var(--xs)', fontWeight: 700, flex: '0 0 auto' }}>{isFix ? '報名' : '加入'}</button>
      </div>
    </div>
  )
}

/* —— 門市抽屜（查看門市）：店頭資訊 + 該店全部即時/固定牌局 + 在這間店開一桌 CTA —— */
function StoreDetailSheet({ store, stakeMap, myCoord, onJoin, onHostHere, onClose }) {
  const [queues, setQueues] = useState(null)   // null=載入中
  ...
  useEffect(() => { let alive = true; fetchMatchQueues(store.id).then((q) => { if (alive) setQueues(q || []) }); return () => { alive = false } }, [store.id])
  const open = isStoreOpen(store.open_time, store.close_time)
  const km = myCoord ? distanceKm(myCoord.lat, myCoord.lng, store.lat, store.lng) : null
  const live = (queues || []).filter((q) => q.source !== 'recurring')
  const fix = (queues || []).filter((q) => q.source === 'recurring')
  return (
    <DragSheet onClose={onClose} title="門市詳情" subtitle="看看這間店現在有哪些牌局，或直接開一桌">
      {/* 門市資訊：淺灰底展示區塊，與上方標題明確分開（純色底展示區塊不加邊框）*/}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--field-bg)', borderRadius: 'var(--r-card)', padding: '12px 14px', marginTop: 10, marginBottom: 16 }}>
        <div style={{ flex: '0 0 46px', width: 46, height: 46, borderRadius: 'var(--r-field)', background: open === false ? 'var(--gray-3)' : 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <MapPin />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 'var(--m)', fontWeight: 700, color: 'var(--ink)', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            {store.name}
            {store.store_type && <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--ink)', background: 'var(--brand)', padding: '1px 7px', borderRadius: 'var(--r-pill)' }}>{storeTypeLabel(store.store_type)}</span>}
            {defaultStoreId && store.id === defaultStoreId && <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--ink)', background: 'var(--brand)', padding: '1px 7px', borderRadius: 'var(--r-pill)' }}>預設</span>}
          </div>
          <div style={{ fontSize: 'var(--xs)', color: 'var(--gray-1)', marginTop: 3 }}>{store.address}</div>
          <div style={{ fontSize: 'var(--xs)', color: 'var(--gray-1)', marginTop: 2 }}>營業 {fmtTime(store.open_time)}–{fmtTime(store.close_time)}{km != null ? ' · ' + fmtDistance(km) : ''}</div>
        </div>
      </div>
      {queues === null ? (
        <p style={{ textAlign: 'center', color: 'var(--gray-2)', fontSize: 'var(--s)', padding: '30px 0' }}>載入中…</p>
      ) : live.length === 0 && fix.length === 0 ? (
        <div style={{ border: '1px dashed var(--field-bd)', background: 'var(--field-bg)', borderRadius: 'var(--r-card)', fontSize: 'var(--xs)', color: 'var(--gray-2)', textAlign: 'center', padding: '16px 0' }}>這間店目前沒有等待中的牌局，開一桌吧！</div>
      ) : <>
        {live.length > 0 && <p style={{ margin: '0 0 8px', fontSize: 13, fontWeight: 600, color: 'var(--gray-1)' }}>即時牌局</p>}
        {live.map((q) => <GameCard key={q.id} q={q} kind="live" stakeMap={stakeMap} onJoin={onJoin} showStore={false} />)}
        {fix.length > 0 && <p style={{ margin: '10px 0 8px', fontSize: 13, fontWeight: 600, color: 'var(--gray-1)' }}>固定牌局</p>}
        {fix.map((q) => <GameCard key={q.id} q={q} kind="fix" stakeMap={stakeMap} onJoin={onJoin} showStore={false} />)}
      </>}
      <div style={{ marginTop: 18 }}>
        <button className="ibtn" onClick={() => onHostHere(store)} style={{ width: '100%', background: 'var(--ink)', color: 'var(--white)', padding: '13px 0', fontSize: 'var(--l)', fontWeight: 700 }}>在這間店開一桌</button>
      </div>
    </DragSheet>
  )
}
function Match({ phase, setPhase, active, setActive, onOpenSettle, onGoStats }) {
  ...
  const [defaultPickerOpen, setDefaultPickerOpen] = useState(false)   // 首頁「切換門市」開的門市篩選抽屜
  const [myCity, setMyCity] = useState(null)          // GPS 定位縣市（失敗則降級用當前門市所在縣市）
  const [storesWithGames, setStoresWithGames] = useState(null)   // 該縣市有等待中牌局的店數；null=尚未統計   // 開一桌切頁裡「切換 ›」開啟的選店抽屜
  ...
  const [defaultStoreId, setDefaultStoreId] = useState(null)   // 預設門市 id
  useEffect(() => { fetchMyProfile().then((p) => { if (p && p.home_store_id) setDefaultStoreId(p.home_store_id) }) }, [])
  const [stores, setStores] = useState([])
  // 開一桌的門市：與首頁檢視門市（storeId）完全獨立，切換互不影響
  const [hostStoreId, setHostStoreId] = useState(null)
  // 預設門市載入後，首頁檢視門市與開一桌門市各自初始化為預設門市（之後各走各的）
  useEffect(() => {
    if (!defaultStoreId || stores.length === 0) return
    const rec = stores.find((x) => x.id === defaultStoreId)
    if (!rec) return
    setStoreId(rec.id)
    setHostStoreId((prev) => prev || rec.id)
    setStore((prev) => ({ ...prev, name: rec.name, area: storeArea(rec), addr: rec.address, lat: rec.lat, lng: rec.lng }))
  }, [defaultStoreId, stores.length])
  ...
  // 定位縣市：GPS 成功用 GPS，失敗降級為當前門市所在縣市
  useEffect(() => {
    if (!navigator.geolocation) return
    navigator.geolocation.getCurrentPosition(
      (pos) => setMyCity(cityFromLatLng(pos.coords.latitude, pos.coords.longitude)),
      () => {}, { timeout: 6000, maximumAge: 300000 }
    )
  }, [])
  const curStoreRec = stores.find((x) => x.id === storeId)
  const cityShown = myCity || curStoreRec?.city || null
  // 「附近 N 家店有牌局」：統計該縣市內有等待中牌局的門市數（跨店，30 秒更新一次）
  useEffect(() => {
    if (!cityShown || stores.length === 0) return
    let alive = true
    const count = () => fetchMatchQueuesByCity(cityShown).then((qs) => {
      if (alive) setStoresWithGames(new Set((qs || []).map((q) => q.store_id)).size)
    })
    count()
    const timer = setInterval(count, 30000)
    return () => { alive = false; clearInterval(timer) }
  }, [cityShown, stores.length])

  // 首頁門市卡：顯示「目前檢視的門市」。預設門市只能在個人設定裡改，
  // 這裡的「切換門市」只換這一次瀏覽的對象，不會寫回 home_store_id
  const defaultStoreRec = stores.find((x) => x.id === defaultStoreId) || null
  const viewStoreRec = stores.find((x) => x.id === storeId) || null
  const hostStoreRec = stores.find((x) => x.id === hostStoreId) || viewStoreRec
  const isViewingDefault = !!storeId && storeId === defaultStoreId
  const [defaultCounts, setDefaultCounts] = useState({ live: 0, fix: 0 })
  useEffect(() => {
    if (!defaultStoreId) return
    // 預設門市就是當前門市時沿用既有 queues，不重複發請求
    if (defaultStoreId === storeId) {
      setDefaultCounts({ live: queues.filter((q) => q.source !== 'recurring').length, fix: queues.filter((q) => q.source === 'recurring').length })
      return
    }
    let alive = true
    const load = () => fetchMatchQueues(defaultStoreId).then((qs) => {
      if (!alive) return
      const list = qs || []
      setDefaultCounts({ live: list.filter((q) => q.source !== 'recurring').length, fix: list.filter((q) => q.source === 'recurring').length })
    })
    load()
    const timer = setInterval(load, 8000)
    return () => { alive = false; clearInterval(timer) }
  }, [defaultStoreId, storeId, queues])
  ...
  const startMatch = async () => {
    if (!hostStoreId) { showToast('門市資料載入中，請稍候'); return }
    ...
    const res = await createMatchQueue({
      storeId: hostStoreId, stakeId: s.id, playAt: playAt.toISOString(), gameType: mahjongKind, flower, rounds,
    ...
    track('table_open', { store: hostStoreRec?.name || store.name, stake: s.label })
    setActive({ store: hostStoreRec?.name || store.name, addr: hostStoreRec?.address || store.addr, stake: s.label, type: mahjongKind, flower, rounds, time: fmtPlayAt(playAt.toISOString()) })
  ...
  const joinReal = async (q) => {
    ...
    // 快照決策當下的環境：剩幾位、距開打多久、是不是固定局。
    // 有這些才答得出「還剩 1 位時報名率是否更高」，事後補不回來。
    const snap = snapQueue({ ...q, store_name: store.name, stake_label: stakeMap[q.stake_level_id] || '' })
    track('join_attempt', snap)
    setActive(card); setTablesPage(false); setJoining(true)
    const res = await joinMatchQueue(q.id, 'browse')
    setJoining(false)
    if (res && res.error) {
      track('join_fail', { ...snap, reason: res.error })
      showToast(res.error === '此桌目前無法加入' ? '此桌目前無法加入' : '加入失敗，請稍後再試'); setActive(null); return
    }
    track('match_join', { ...snap, status: res && res.status })
  ...
  const cancelWith = (reason) => {
    ...
    track('join_cancel', {
      queue_id: myQueue?.id, reason,
      minutes_to_play: myQueue?.play_at
        ? Math.round((new Date(myQueue.play_at) - Date.now()) / 60000) : null,
    })
    if (myQueue?.id) leaveMatchQueue(myQueue.id, reason)
    setMyQueue(null); setPhase('idle'); setActive(null); setSetupOpen(false); showToast('已取消配桌')
  }
      ...
      <div style={{ background: 'var(--brand-light)', border: '1.5px solid var(--brand)', borderRadius: 'var(--r-lg)', padding: '14px 16px' }}>
        {phase === 'waiting' ? <>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {(() => {
              // 以伺服器的 myQueue 為準，active 只當還沒輪詢到時的暫時值
              // active（加入/開桌當下取自牌局資料，欄位最完整）優先；myQueue 只補 active 沒有的洞
              const gType = active?.type || myQueue?.game_type
              const gFlower = active?.flower || myQueue?.flower
              const gRounds = active?.rounds || myQueue?.rounds
              const gStake = active?.stake || (myQueue?.stake_level_id && stakeMap[myQueue.stake_level_id]) || ''
              const gTime = active?.time || (myQueue?.play_at ? fmtPlayAt(myQueue.play_at) : '')
              const play = [fmtType(gType, gFlower), gStake, gRounds].filter(Boolean).join(' · ')
              return [['門市', active?.store || store.name], ['玩法', play || '—'], ['開打時間', gTime || '—']]
            })().map(([k, v], idx, arr) => (
    ...
    <>
      ...
      {!inProgress && <>
        {cityShown && (
          <div style={{ padding: '12px 20px 0', display: 'flex', alignItems: 'center', gap: 6 }}>
            <MapPin /><span style={{ fontSize: 'var(--s)', color: 'var(--gray-1)' }}>你在 <b style={{ color: 'var(--ink)', fontWeight: 700 }}>{cityShown}</b>{storesWithGames != null && (storesWithGames > 0 ? ` · 附近 ${storesWithGames} 家店有牌局` : ' · 附近暫時沒有牌局')}</span>
          </div>
        )}

        {!defaultStoreRec && !viewStoreRec && (
          <div style={{ padding: '12px 20px 0' }}>
            <p style={{ margin: '0 0 8px', fontSize: 'var(--l)', fontWeight: 700, color: 'var(--ink)' }}>我的預設門市</p>
            <div style={{ background: 'var(--field-bg)', border: '1px solid var(--field-bd)', borderRadius: 'var(--r-card)', padding: '16px 0', textAlign: 'center', fontSize: 'var(--s)', color: 'var(--gray-1)' }}>你還沒有設定預設門市</div>
          </div>
        )}
        {viewStoreRec && (
          <div style={{ padding: '12px 20px 0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ margin: '0 0 8px', fontSize: 'var(--l)', fontWeight: 700, color: 'var(--ink)' }}>{isViewingDefault ? '我的預設門市' : '目前門市'}</p>
              <span onClick={() => setDefaultPickerOpen(true)} style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)', cursor: 'pointer' }}>切換門市 ›</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--field-bg)', border: '1px solid var(--field-bd)', borderRadius: 'var(--r-card)', padding: '12px 14px' }}>
              <div style={{ flex: '0 0 46px', width: 46, height: 46, borderRadius: 'var(--r-field)', background: isStoreOpen(viewStoreRec.open_time, viewStoreRec.close_time) === false ? 'var(--gray-3)' : 'var(--brand)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <MapPin />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 'var(--m)', fontWeight: 700, color: 'var(--ink)', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  {viewStoreRec.name}
                  {viewStoreRec.store_type && <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--ink)', background: 'var(--brand)', padding: '1px 7px', borderRadius: 'var(--r-pill)' }}>{storeTypeLabel(viewStoreRec.store_type)}</span>}
                </div>
                <div style={{ fontSize: 'var(--xs)', color: 'var(--gray-1)', marginTop: 3 }}>
                  {defaultCounts.live + defaultCounts.fix === 0 ? '目前沒有等待中的牌局' : `即時 ${defaultCounts.live} 局 · 固定 ${defaultCounts.fix} 局`}
                </div>
              </div>
            </div>
          </div>
        ...
        <div style={{ padding: '14px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <p style={{ margin: '0 0 8px', fontSize: 'var(--l)', fontWeight: 700, color: 'var(--ink)' }}>這間店的牌局</p>
          </div>
          ...
          {liveQueues.slice(0, 2).map((q) => <GameCard key={q.id} q={q} kind="live" stakeMap={stakeMap} onJoin={joinReal} showStore={false} />)}
          {fixQueues.length > 0 && <p style={{ margin: '10px 0 8px', fontSize: 13, fontWeight: 600, color: 'var(--gray-1)' }}>固定牌局</p>}
          {fixQueues.slice(0, 2).map((q) => <GameCard key={q.id} q={q} kind="fix" stakeMap={stakeMap} onJoin={joinReal} showStore={false} />)}
          <button className="ibtn" onClick={() => setTablesPage(true)} style={{ width: '100%', background: 'none', color: 'var(--accent)', padding: '12px 0 4px', fontSize: 'var(--m)', fontWeight: 700, marginTop: 2 }}>查看更多門市的牌局 ›</button>
        </div>
      ...
      {setupOpen && createPortal(
        <div className="sheet-anim" style={{ position: 'fixed', inset: 0, maxWidth: 480, margin: '0 auto', background: 'var(--white)', zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 16px 12px', borderBottom: '.5px solid var(--gray-4)' }}>
            <button onClick={() => setSetupOpen(false)} style={{ background: 'none', border: 'none', fontSize: 22, color: 'var(--ink)', cursor: 'pointer', padding: '0 4px' }}>‹</button>
            <span style={{ fontSize: 17, fontWeight: 700, color: 'var(--ink)' }}>我要自己開一桌</span>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 20px 20px' }}>
            <div style={{ padding: '8px 0 0' }}><p style={{ margin: '0 0 8px', fontSize: 'var(--l)', fontWeight: 700, color: 'var(--ink)' }}>要在哪開桌</p>
              <div onClick={() => setHostPickerOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--field-bg)', border: '1px solid var(--field-bd)', borderRadius: 'var(--r-card)', padding: '12px 14px', cursor: 'pointer' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 'var(--m)', fontWeight: 700, color: 'var(--ink)', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    {hostStoreRec?.name || store.name}
                    {defaultStoreId && hostStoreId === defaultStoreId && <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--ink)', background: 'var(--brand)', padding: '1px 7px', borderRadius: 'var(--r-pill)' }}>預設</span>}
                  </div>
                  <div style={{ fontSize: 'var(--xs)', color: 'var(--gray-1)', marginTop: 2 }}>{hostStoreRec?.address || store.addr}</div>
                </div>
                <span style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 700, flex: '0 0 auto', whiteSpace: 'nowrap' }}>切換 ›</span>
              </div>
            </div>
          ...
          {hostPickerOpen && (
            <DragSheet onClose={() => setHostPickerOpen(false)} title="選擇門市">
              <StorePicker mode="host" current={hostStoreRec?.name} onPick={(x) => { setHostStoreId(x.id); setHostPickerOpen(false) }} />
            </DragSheet>
          )}
        </div>, document.body
      )}
      ...
      {defaultPickerOpen && (
        <DragSheet onClose={() => setDefaultPickerOpen(false)} title="切換門市" subtitle="選擇你要切換的門市">
          <StorePicker mode="match" current={viewStoreRec?.name} onPick={(x) => { setStoreId(x.id); setStore((prev) => ({ ...prev, name: x.name, area: storeArea(x), addr: x.address, lat: x.lat, lng: x.lng })); setDefaultPickerOpen(false) }} />
        </DragSheet>
      )}
      {tablesPage && <TablesPage stores={stores} stakeMap={stakeMap} onJoin={joinReal} onHostHere={(s) => { setHostStoreId(s.id); setTablesPage(false); setSetupOpen(true) }} onClose={() => setTablesPage(false)} />}
...
// 開始配桌（UberEats 式 feed）：縣市/區域雙下拉決定瀏覽範圍，跨店牌局混排；
// 點牌局卡的店頭列 → 查看門市抽屜（該店全部牌局＋在這間店開一桌 CTA）
function TablesPage({ stores, stakeMap, onJoin, onHostHere, onClose }) {
  const [filt, setFilt] = useState('全部')
  const [myCoord, setMyCoord] = useState(null)
  // 縣市預設值：GPS 定位到的縣市（且該縣市有門市）→ 否則「全部縣市」
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setMyCoord({ lat: pos.coords.latitude, lng: pos.coords.longitude })
        const c = cityFromLatLng(pos.coords.latitude, pos.coords.longitude)
        // 定位到的縣市有門市才套用，否則維持「全部縣市」
        if (c) setCity((prev) => (prev === '全部' ? c : prev))
      },
      () => { /* 拒絕定位或失敗 → 維持「全部縣市」，不擋 UI */ },
      { timeout: 6000, maximumAge: 300000 }
    )
  }, [])
  useEffect(() => {
    // 定位縣市在門市清單裡不存在時（例如該縣市還沒展店）降級回「全部縣市」
    if (CITY_LIST.length > 0 && city !== '全部' && !CITY_LIST.includes(city)) { setCity('全部'); setArea('全部') }
  }, [stores.length, city])
  const areas = ['全部', ...Array.from(new Set(stores.filter((s) => city === '全部' || s.city === city).map((s) => storeArea(s)).filter(Boolean)))]

  // 跨店牌局：一次向後端要整個縣市（可再限定行政區）的牌局，不再逐店發請求
  const [cityQueues, setCityQueues] = useState([])
  const [loading, setLoading] = useState(true)
  useEffect(() => {
    let alive = true
    setLoading(true)
    const load = () => fetchMatchQueuesByCity(city, area).then((qs) => { if (alive) { setCityQueues(qs || []); setLoading(false) } })
    load()
    const timer = setInterval(load, 8000)
    return () => { alive = false; clearInterval(timer) }
  }, [city, area])

  // RPC 已帶回門市欄位；仍以門市清單為主來源，缺漏時用 RPC 帶回的值補
  const storeMap = Object.fromEntries(stores.map((s) => [s.id, s]))
  const allQueues = cityQueues.map((q) => ({
    q,
    store: storeMap[q.store_id] || { id: q.store_id, name: q.store_name, city: q.store_city, area: q.store_area, lat: q.store_lat, lng: q.store_lng },
  }))
  // 對齊定案：即時牌局在前、固定牌局在後分區排列；「全部」時各帶區塊標題，單選類型時不帶
  const showLive = filt !== '固定', showFix = filt !== '即時'
  const liveList = showLive ? allQueues.filter((x) => x.q.source !== 'recurring') : []
  const fixList = showFix ? allQueues.filter((x) => x.q.source === 'recurring') : []

  const [storeSheet, setStoreSheet] = useState(null)   // 查看門市抽屜（門市物件）
  ...
  return createPortal(
    <div className="sheet-anim" style={{ position: 'fixed', inset: 0, maxWidth: 480, margin: '0 auto', background: 'var(--white)', zIndex: 1000, display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 16px 12px', borderBottom: '.5px solid var(--gray-4)' }}>
        <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 22, color: 'var(--ink)', cursor: 'pointer', padding: '0 4px' }}>‹</button>
        <span style={{ fontSize: 17, fontWeight: 700, color: 'var(--ink)' }}>開始配桌</span>
      </div>
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 20px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', marginBottom: 8 }}>
          {['全部', '即時', '固定'].map((f) => <span key={f} onClick={() => setFilt(f)} style={{ flex: '0 0 auto', border: filt === f ? '1px solid var(--brand)' : '1px solid var(--gray-3)', background: filt === f ? 'var(--brand)' : 'var(--white)', color: filt === f ? 'var(--ink)' : 'var(--gray-1)', fontWeight: filt === f ? 600 : 400, borderRadius: 'var(--r-pill)', padding: '6px 16px', fontSize: 13, cursor: 'pointer' }}>{f}</span>)}
        </div>
        {loading ? (
          <p style={{ textAlign: 'center', color: 'var(--gray-2)', fontSize: 13, padding: '48px 0' }}>載入中…</p>
        ) : liveList.length === 0 && fixList.length === 0 ? (
          <div style={{ border: '1px dashed var(--field-bd)', background: 'var(--field-bg)', borderRadius: 'var(--r-card)', fontSize: 'var(--xs)', color: 'var(--gray-2)', textAlign: 'center', padding: '16px 0', marginBottom: 8 }}>這裡目前沒有{filt === '全部' ? '等待中的' : filt}牌局，開一桌吧！</div>
        ) : <>
          {liveList.length > 0 && filt === '全部' && <p style={{ margin: '10px 0 8px', fontSize: 13, fontWeight: 600, color: 'var(--gray-1)' }}>即時牌局</p>}
          {liveList.map(({ q, store: s }) => (
            <GameCard key={q.id} q={q} kind="live" store={s} stakeMap={stakeMap} myCoord={myCoord} onJoin={onJoin} onOpenStore={setStoreSheet} />
          ))}
          {fixList.length > 0 && filt === '全部' && <p style={{ margin: '10px 0 8px', fontSize: 13, fontWeight: 600, color: 'var(--gray-1)' }}>固定牌局</p>}
          {fixList.map(({ q, store: s }) => (
            <GameCard key={q.id} q={q} kind="fix" store={s} stakeMap={stakeMap} myCoord={myCoord} onJoin={onJoin} onOpenStore={setStoreSheet} />
          ))}
        </>}
      </div>
      {storeSheet && (
        <StoreDetailSheet
          ...
          stakeMap={stakeMap}
          myCoord={myCoord}
          onJoin={(q) => { onJoin(q); setStoreSheet(null) }}
          onHostHere={(s) => { setStoreSheet(null); onHostHere(s) }}
