...
function Wallet({ member, go, matching = false, matchInfo = null }) {
  ...
  const txns = d?.txns || []
  const tile = (t) => ({ table_fee: ['桌', 'var(--brand)'], fnb: ['茶', 'var(--milktea)'], topup: ['儲', '#FBEAF0'], merch: ['物', 'var(--gray-4)'], event_fee: ['活', 'var(--milktea)'] }[t] || ['點', 'var(--gray-4)'])
  ...
  return (
    <>
      ...
      <div style={{ padding: '0 20px' }}>
        <div style={{ background: 'var(--brand)', borderRadius: 22, padding: '17px 18px 15px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 13 }}>
            <button className="ibtn" style={{ flex: 1, background: 'var(--ink)', padding: '10px 0', fontSize: 'var(--m)', fontWeight: 700, color: 'var(--white)' }} onClick={() => setTopup(true)}>儲值</button>
      ...
      {/* 配桌中提示（僅配桌中顯示；閒置不顯示招攬卡）*/}
      {matching && (
      <div style={{ padding: '14px 20px 0' }}>
          <div onClick={() => go(1)} style={{ display: 'flex', alignItems: 'center', gap: 10, border: '1.5px solid var(--brand)', borderRadius: 'var(--r-card)', padding: '11px 13px', background: 'var(--brand-light)', cursor: 'pointer' }}>
            <span className="spin" style={{ width: 18, height: 18, border: '2.5px solid #F3D9E1', borderTopColor: 'var(--accent)', borderRadius: '50%', display: 'inline-block', flex: '0 0 18px' }} />
            <div style={{ flex: 1 }}>
              <p style={{ margin: 0, fontSize: 'var(--s)', fontWeight: 700, color: 'var(--ink)' }}>配桌中 · 2/4 · 還差 2 位</p>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: '#8a5563' }}>配好會通知你 · 點我回配桌頁</p>
            </div>
            <span style={{ fontSize: 16, color: '#A4566A' }}>›</span>
          </div>
      </div>
      )}
      ...
      <div style={{ padding: '0 20px 16px' }}>
        {txns.length === 0 ? (
          <p style={{ margin: 0, fontSize: 'var(--s)', color: 'var(--gray-1)' }}>還沒有紀錄</p>
        ) : txns.map((t, i) => {
      ...
      {sheet && (
        <DragSheet onClose={() => setSheet(false)} title="優惠券">
          ...
          {coupons.length === 0 ? (
            <p style={{ padding: 20, textAlign: 'center', color: 'var(--gray-2)', fontSize: 12 }}>目前沒有優惠券</p>
...
function TopupSheet({ balance, onClose }) {
  ...
  function confirm() {
    if (pay === '現金支付') return alert('請到櫃台付現，由店員為你加值')
    alert('儲值金流串接後開放（' + pay + ' · ' + amt.toLocaleString() + ' 點）')
  }
  return (
    <DragSheet onClose={onClose} title="儲值點數" subtitle="點數用於檯費、餐飲、派車等服務 · 不可提領、不可換現">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
        {AMTS.map((a) => (
          <div key={a.v} onClick={() => setAmt(a.v)}
...
function Statement({ balance, txns, onClose }) {
  ...
  const tile = (t) => ({ table_fee: ['桌', 'var(--brand)'], fnb: ['茶', 'var(--milktea)'], topup: ['儲', '#FBEAF0'], merch: ['物', 'var(--gray-4)'], event_fee: ['活', 'var(--milktea)'], adjust: ['贈', '#FBEAF0'] }[t] || ['點', 'var(--gray-4)'])
