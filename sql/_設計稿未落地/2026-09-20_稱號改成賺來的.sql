-- ============================================================
-- ⏸ 未落地 —— **2026-09-20 當天就被推翻，不要跑這一份**
-- ============================================================
-- 使用者先選 B（賺來的），同一天改回 **A：稱號註冊預設就有**。
-- ⇒ 現況（`NOT NULL DEFAULT '新手上路'`）**本來就是 A**，一行都不用改。
--
-- 🎯 而 A 案下，我在下面罵的那「四個地方寫死」**全部都是對的**：
-- ```
-- set_my_title_tx 的放行      ✅ 新手上路是「預設」不是「解鎖的」
-- member_app_state.titles=[]  ✅ 那是「解鎖清單」，預設稱號本來就不該在裡面
-- rewards.jsx「註冊就有」      ✅ 它就是在描述事實
-- profile.jsx 的 fallback      ✅ 接的是「還在載入」不是「沒有稱號」
-- ```
-- ⇒ 重複仍然存在（改名要動四處），但**沒有一處在說謊**。
--   在 B 案下那是缺陷，在 A 案下那是一致的重複 —— **同樣的四處，定性完全相反**。
--
-- ### ✅ 但這一份留著，因為裡面有一段今天就缺的東西
-- 步驟 ④：**`ach_unlock_tx` 解鎖時真的把 `grants_title` 寫進去**。
-- `achievements.grants_title` 欄位早就存在、`ach_unlock_tx` 也已經把它
-- **回傳**出來，但**沒有任何東西拿它去寫 `members.title`**
-- ⇒ 哪一天有第一枚「解鎖就送稱號」的成就（M4 的旗艦 MIGI 就是
--   `grants_title = 'MIGI'`），要接的就是這一段。
-- ⚠ 屆時**只取步驟 ③④**，不要碰 ①②⑤（那三步是 B 案專用的）。
--
-- ⚠ **「只在 members.title is null 時才自動配戴」那條仍然成立**，
--   而且在 A 案下更重要：每個人一註冊就戴著新手上路 ⇒ `title` 永遠不是 null
--   ⇒ **自動配戴永遠不會發生**，解鎖只會加進清單，由客人自己去換。
--   🎯 那其實正是想要的行為。
-- ============================================================
--
-- （以下為原始的 B 案設計，保留供對照。）
--
-- ============================================================
-- 稱號改成「賺來的」，不是註冊就有（2026-09-20 使用者選 B 案 → 同日推翻）
--
-- ============================================================
