/* ============================================================
   ...
   **Postgres 建立函式時預設就把 EXECUTE 授權給 `PUBLIC`**，而 `PUBLIC`
   ...
   ── 為什麼只收這 12 支 ────────────────────────────
   全庫 138 支裡：
     · **124 支有明確的 `anon` 授權** —— 那是刻意給的（前端要叫），不動
     · **12 支只靠 PUBLIC 進來** —— 從來沒有人「決定」要給 anon
     · **2 支兩邊都沒有**（`charge_matched_tx` / `charge_private_tx`）——
       純內部呼叫，本來就不該給任何前端
   → 收 PUBLIC 只影響那 12 支，124 支的 anon 是明確授權，不受影響。

   ⚠ **第一次跑的時候 ④ 的期望值寫成 126（＝138 − 12），那是算錯的** ——
     那個減法漏掉了「兩邊都沒有」的 2 支。驗證因此回報「🔴 數字變了」
     而實際上一支都沒少。**儀器沒錯，是寫進去的期望值錯了。**
   📌 留著這段是因為它示範了正對照的另一面：
     **期望值本身也要有來源，不能用心算推。**
...
select 序, 項目, 內容 from (
  ...
  /* 🎯 第二個正對照：確認沒有把前端要用的那 126 支一起收掉。
        跑之前是 126，跑之後也要是 126。 */
  select 4, '④ 🎯 正對照：明確授權 anon 的函式數（跑之前 124，之後也要 124）',
         (select count(*)::text || ' 支'
              || case when count(*) = 124 then '　✅ 一支都沒少'
