...
select 序, 項目, 內容 from (
  ...
  select 6, '⑥ 結構物件數 vs baseline（2026-09-19：表51 函式232 索引98 policy29 帶can20）',
         (select '表 ' || (select count(*)::text from pg_class c
              ...
              || case when (select count(*) from pg_class c
                             join pg_namespace n on n.oid=c.relnamespace
                            where n.nspname='public' and c.relkind='r') = 51
                       and (select count(*) from pg_proc p
                             where p.pronamespace='public'::regnamespace and p.prokind='f') = 232
  ...
  /* ⑨ 上線清單還剩幾項。
        ...
        🔴 **但「固定列出來」有代價，而 2026-09-10 付了一次**：
          那兩項 09-05 就完成了，這裡卻印了五天的 ⏳。
          ⇒ **寫死的格子會過期，而且不會有人發現** ——
            同硬規則 1.6「靠『記得更新』已經被證明行不通」。
          → 所以那兩行現在**附上一行 curl**：讓下一個人能在五秒內
            自己確認，而不是相信一句話。 */
  select 9, '⑨ 上線清單（📄 docs/09-環境流程/上線當天要設的東西.md）',
         (select string_agg(項, E'\n') from (
            ...
            /* 🔴 這兩項 SQL 查不到，但**看不到不等於不用做**。
               ⚠ 它們**不是檢查，是備忘** —— 沒有任何東西會讓它們自己變綠。

               🔴 **2026-09-10 更正：兩項在 2026-09-05 就都做完了，
                 而這兩行從那天起整整五天都還印著 ⏳。**
                 也就是每一次 session 開頭都會看到兩個假的待辦。
               🎯 **那正是這個專案記過三次的病**（硬規則 3.5 第三次、
                 3.6c、PENDING 那個 break-glass 帳號的紅字）：
                 **一個永遠不會變綠的格子，會訓練人略過整份儀表。**
                 ⚠ 而它比「沒有這一格」更糟：它讓人以為儀表在盯著。

               ✅ 查證方式（一行，不需要資料庫）：
               ```bash
               curl -s -o /dev/null -w "%{http_code}" https://migi.tw/privacy
               curl -s -o /dev/null -w "%{http_code}" -L https://line.me/R/ti/p/@254blful
               ```
               兩個都回 200 就是還活著。⚠ **下一個人請照著跑，不要相信這兩行字** ——
               它們跟 2026-09-05 之前那兩行一樣，只是有人打上去的。 */
            select 6, '✅ ⑥ LINE 官方帳號 @254blful（2026-09-05；⏳ 只剩圖文選單，見待辦 38）'
            union all
            select 7, '✅ ⑦ 隱私權政策與服務條款 migi.tw/privacy · /terms（App 已接，profile.jsx:498）'
            union all
